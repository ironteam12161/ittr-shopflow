...
// v24.24.3 lazy-route global action exports
...
window.runSamsaraDiagnostics=async function(){const b=document.getElementById('samsaraSyncSummary'),r=document.getElementById('samsaraSyncReview');try{if(b)b.textContent='Checking Railway database, Samsara token, API and unit-sync schema…';const resp=await fetch('/api/samsara/diagnostics',{credentials:'same-origin',cache:'no-store'});const d=await resp.json();const missing=Object.entries(d.columns||{}).filter(([,v])=>!v).map(([k])=>k);if(b)b.innerHTML=`<b>Build ${esc(d.build||'—')}</b> · Database ${d.database?'✓':'✗'} · Token ${d.tokenConfigured?'✓':'✗'} · Samsara API ${d.samsaraApi?'✓':'✗'} · Vehicles ${Number(d.vehicleCount||0)}`;if(r)r.innerHTML=(missing.length?`<div class="notice" style="margin-top:8px"><b>Missing database columns:</b> ${missing.map(esc).join(', ')}</div>`:'')+(d.errors||[]).map(x=>`<div class="notice" style="margin-top:8px;color:#b42318"><b>Diagnostic:</b> ${esc(x)}</div>`).join('')+((d.ok)?'<div class="notice" style="margin-top:8px;color:#067647"><b>Connection and database checks passed.</b> You can Preview Unit Matches.</div>':'')}catch(e){if(b)b.textContent='Diagnostic failed: '+(e.message||String(e))}};
