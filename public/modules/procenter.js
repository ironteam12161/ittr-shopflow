...
// v24.24.5 lazy-route global action exports
...
window.runSamsaraDiagnostics=async function(){const b=document.getElementById('samsaraSyncSummary'),r=document.getElementById('samsaraSyncReview');try{if(b)b.textContent='Checking running build, Railway database, Samsara token, schema and Samsara API…';const resp=await fetch('/api/samsara/diagnostics',{credentials:'same-origin',cache:'no-store',headers:window.authHeaders?.({Accept:'application/json'})||{Accept:'application/json'}});let d;try{d=await resp.json()}catch(_){throw new Error(`Diagnostics returned HTTP ${resp.status} instead of JSON`)}const env=d.env||{},db=d.database||{},sa=d.samsara||{},sc=d.schema||{},mk=v=>v?'✓':'✗';if(b)b.innerHTML=`<b>Build ${esc(d.build||'UNKNOWN')}</b> · DATABASE_URL ${mk(env.databaseUrlPresent)} · Database ${mk(db.ok)} · SAMSARA_API_TOKEN ${mk(env.samsaraTokenPresent)} · Schema ${mk(sc.ok)} · Samsara API ${mk(sa.ok)} · Vehicles ${Number(sa.vehicleCount||0)}`;const x=[];if(!env.databaseUrlPresent)x.push('DATABASE_URL is not visible to this running Railway service.');if(env.databaseUrlPresent&&!db.ok)x.push('Database connection failed: '+(db.reason||'unknown error'));if(sc.reason)x.push('Schema check failed: '+sc.reason);if(sc.missing?.length)x.push('Missing unit columns: '+sc.missing.join(', '));if(!env.samsaraTokenPresent)x.push('SAMSARA_API_TOKEN is not visible to this running Railway service.');if(env.samsaraTokenPresent&&!sa.ok)x.push(`Samsara API failed${sa.httpStatus?' (HTTP '+sa.httpStatus+')':''}: ${sa.reason||'unknown error'}`);if(r)r.innerHTML=x.map(v=>`<div class="notice" style="margin-top:8px;color:#b42318"><b>Diagnostic:</b> ${esc(v)}</div>`).join('')+(d.ok?`<div class="notice" style="margin-top:8px;color:#067647"><b>All checks passed.</b> ${Number(sa.vehicleCount||0)} Samsara vehicle(s) returned. Run Preview Unit Matches next.</div>`:'')}catch(e){if(b)b.innerHTML=`<span style="color:#b42318"><b>Diagnostic failed:</b> ${esc(e.message||String(e))}</span>`}};