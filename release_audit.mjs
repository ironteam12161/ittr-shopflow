...
check('release package version is 24.24.4', pkg.version === '24.24.4', pkg.version);
check('frontend release is 24.24.4', html.includes("const FRONTEND_VERSION='24.24.4';") || html.includes('const FRONTEND_VERSION="24.24.4"'));
check('backend release is 24.24.4', server.includes('frontendExpected:"24.24.4",backend:"24.24.4"'));
...
check('v24.24.4 route modules use explicit cache-busting version', html.includes('ROUTE_MODULE_VERSION="24.24.4"') && html.includes('routeAsset("/modules/procenter.html")'));
...
check('invoice v24.24.4 layout hardening exists', modulesInvoicesHtml.includes("invoice-v24211-layout-hardening"));
...
check('app shell globally loads invoice workspace CSS', html.includes('/invoice-workspace.css?v=24.24.4'));
...
check('duplicate audit uses real customer DOT schema column', server.includes('c.dot_number AS usdot') && !server.includes('c.usdot,c.fullbay_id'));
check('customer merge relinks invoices and service orders', server.includes('UPDATE customer_invoices SET customer_id=$1') && server.includes('UPDATE service_orders SET customer_id=$1'));
...
check('v24.24.4 duplicate merge actions exported from lazy route', fs.readFileSync('public/modules/procenter.js','utf8').includes('window.previewDuplicateMerge = previewDuplicateMerge') && fs.readFileSync('public/modules/procenter.js','utf8').includes('window.commitDuplicateMerge = commitDuplicateMerge'));
...
check('merge preview reports failing stage', server.includes("Customer merge preview failed during ${stage}."));
check('merge handles optional invoice/service-order tables safely', server.includes("to_regclass('public.customer_invoices')") && server.includes("to_regclass('public.service_orders')"));

check('merge preview POST serializes JSON body', fs.readFileSync('public/modules/procenter.js','utf8').includes("body:JSON.stringify({masterId,duplicateId})"));
check('apiJSON safely normalizes plain object bodies', html.includes('const normalizedBody=options.body&&typeof options.body==="object"'));
check('merge commit POST serializes JSON body', (fs.readFileSync('public/modules/procenter.js','utf8').match(/body:JSON\.stringify\(\{masterId,duplicateId\}\)/g)||[]).length >= 2);

check('invoice unit history is marked completed independently of billing status', server.includes('status:"Completed",invoiceStatus:x.status'));
check('invoice history includes parent line linkage for parts', server.includes("'parentLineId',l.parent_line_id"));
check('invoice history includes detailed part pricing', server.includes("'unitCost',l.unit_cost") && server.includes("'unitPrice',l.unit_price"));
check('unit history opens read-only invoice service detail', html.includes('function openInvoiceServiceHistory(invoiceId)') && html.includes('Service history is read-only.'));
check('invoice service detail groups Parts Used under labor', html.includes('Parts Used') && html.includes('p.parentLineId'));
check('invoice service history edit path points only to original invoice', html.includes('Edit Original Invoice') && html.includes('openInvoiceWorkspace(${Number(w.id)})'));

check('Fullbay history cleanup audit is owner-only', server.includes("app.get('/api/fullbay/history/import-audit',auth,ownerOnly"));
check('Fullbay history cleanup delete is owner-only', server.includes("app.post('/api/fullbay/history/delete-imported',auth,ownerOnly"));
check('Fullbay history cleanup requires exact typed confirmation', server.includes("confirmation!=='DELETE FULLBAY HISTORY'"));
check('Fullbay cleanup targets imported source keys only', server.includes("DELETE FROM fullbay_service_history WHERE coalesce(source_key,'') LIKE 'fullbay:%'"));
check('Fullbay cleanup UI preserves ITTR records', fs.readFileSync('public/modules/procenter.html','utf8').includes('does not delete ITTR invoices, work orders, customers, units, inventory'));

check('barcode camera scan interval reduced to 120ms', html.includes('}},120);return}if(window.Html5Qrcode)'));
check('barcode camera requests HD environment camera', html.includes("width:{ideal:1280},height:{ideal:720}"));
check('barcode camera attempts continuous autofocus', html.includes("focusMode:'continuous'"));
check('barcode scanner suppresses repeated reads', html.includes("partsScannerLastCode") && html.includes("now-partsScannerLastAt<1200"));
check('manufacturer barcode lookup has GIN index', server.includes('idx_fullbay_parts_barcode_aliases_gin'));
check('manufacturer barcode lookup uses indexed JSON containment first', server.includes("barcode_aliases @> jsonb_build_array($1::text)"));

check('invoice service history uses real customer_invoices tax column', server.includes('i.tax AS tax_amount'));
check('invoice service history derives additional fees from real schema', server.includes("coalesce(i.shop_supplies,0)+coalesce(i.environmental_fee,0)"));
check('invoice service history no longer queries nonexistent tax_amount column', !server.includes('i.tax_amount,i.discount,i.additional_fees,i.other_charges'));
check('invoice service history keeps all non-void billing statuses', server.includes("WHERE i.status<>'void'"));
check('invoice history matches unit by unit_id or unit/customer identity', server.includes("i.unit_id=$1::bigint OR (lower(coalesce(i.unit_number,''))=lower($2)"));

check('invoice history modal is in live app DOM after final script', html.indexOf('id="invoiceHistoryDetailModal"') > html.lastIndexOf('</script>'));
check('invoice history title exists in live app DOM', html.indexOf('id="invoiceHistoryDetailTitle"') > html.lastIndexOf('</script>'));
check('invoice history body exists in live app DOM', html.indexOf('id="invoiceHistoryDetailBody"') > html.lastIndexOf('</script>'));
check('invoice history opener has null DOM guard', html.includes('Invoice history modal DOM is unavailable'));
check('work order modal remains in live DOM', html.includes('id="woModal"'));
check('work order detail modal remains in live DOM', html.includes('id="detailModal"'));
