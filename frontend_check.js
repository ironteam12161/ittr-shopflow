...
document.addEventListener("keydown",e=>{
 if(e.key==="Escape"){
   document.querySelectorAll(".modal.open").forEach(m=>m.classList.remove("open")); updateMobileBackButton();
 }
});
...
let partsSearchTimer=null,partsScannerStream=null,partsScannerTimer=null,partsHtml5Scanner=null,partsScannerContext={mode:"lookup",workOrderId:null,taskIndex:null};
function partStockClass(x){const q=Number(x.quantity||0),r=Number(x.reorder_point??x.min_qty??0);return q<=0?"stockOut":(r>0&&q<=r?"stockWarn":"stockGood")}
async function loadPartsCenter(){const list=document.getElementById("partsList");if(!list)return;const q=document.getElementById("partsSearch")?.value?.trim()||"",filter=document.getElementById("partsFilter")?.value||"all";list.innerHTML='<div class="muted">Loading inventory…</div>';try{const [sum,d]=await Promise.all([apiJSON('/api/parts/summary'),apiJSON(`/api/parts?q=${encodeURIComponent(q)}&filter=${encodeURIComponent(filter)}&limit=150`)]);const stats=document.getElementById("partsStats");if(stats)stats.innerHTML=`<div class="partsStat"><span class="muted">Parts</span><b>${Number(sum.total||0).toLocaleString()}</b></div><div class="partsStat"><span class="muted">Low Stock</span><b>${Number(sum.low_stock||0).toLocaleString()}</b></div><div class="partsStat"><span class="muted">Out of Stock</span><b>${Number(sum.out_of_stock||0).toLocaleString()}</b></div><div class="partsStat"><span class="muted">On Order</span><b>${Number(sum.on_order||0).toLocaleString()}</b></div><div class="partsStat"><span class="muted">Inventory Value</span><b>${money(sum.inventory_value)}</b></div>`;const items=d.items||[];list.innerHTML=items.length?`<table class="partsTable"><thead><tr><th>Part</th><th>Description</th><th>Available</th><th>Allocated</th><th>Location</th><th>Price</th><th></th></tr></thead><tbody>${items.map(x=>`<tr><td><b>${esc(cleanImportedDisplayText(x.part_number)||'—')}</b><div class="muted">${esc(x.manufacturer||'')}${x.internal_barcode?` · ${esc(x.internal_barcode)}`:''}</div></td><td>${esc(cleanImportedDisplayText(x.description)||'—')}<div class="muted">${esc(x.category||'')}</div></td><td class="${partStockClass(x)}">${Number(x.available||0).toLocaleString()} ${esc(x.uom||'')}</td><td>${Number(x.allocated||0).toLocaleString()}</td><td>${esc(x.location||'—')}</td><td>${money(x.price)}</td><td><button class="secondary" onclick="openPartDetail(${Number(x.id)})">Open</button></td></tr>`).join('')}</tbody></table>`:'<div class="muted">No parts found.</div>'}catch(e){list.innerHTML=`<div class="error">${esc(e.message||'Unable to load parts.')}</div>`}}
function debouncedPartsSearch(){clearTimeout(partsSearchTimer);partsSearchTimer=setTimeout(loadPartsCenter,180)}
async function openPartDetail(id){const m=document.getElementById('partDetailModal'),body=document.getElementById('partDetailBody');m?.classList.add('open');syncModalState();body.innerHTML='<div class="muted">Loading…</div>';try{const d=await apiJSON(`/api/parts/${id}`),x=d.item,tx=d.transactions||[];document.getElementById('partDetailTitle').textContent=`${cleanImportedDisplayText(x.part_number)||'Part'} — ${cleanImportedDisplayText(x.description)||''}`;const aliases=Array.isArray(x.barcode_aliases)?x.barcode_aliases:[];body.innerHTML=`<div class="partDetailGrid"><div><div class="card"><h3>${esc(cleanImportedDisplayText(x.part_number)||'—')}</h3><div>${esc(cleanImportedDisplayText(x.description)||'—')}</div><div class="muted">${esc(x.manufacturer||'')} · ${esc(x.category||'')}</div><hr><div class="customerDirectoryGrid"><div><span class="muted">On Hand</span><b class="${partStockClass(x)}">${Number(x.quantity||0).toLocaleString()}</b></div><div><span class="muted">Allocated</span><b>${Number(x.allocated||0).toLocaleString()}</b></div><div><span class="muted">Available</span><b>${Number(x.available||0).toLocaleString()}</b></div><div><span class="muted">On Order</span><b>${Number(x.on_order||0).toLocaleString()}</b></div></div><hr><div><b>Location:</b> ${esc(x.location||'—')} &nbsp; <b>Vendor:</b> ${esc(x.vendor||'—')}</div><div><b>Avg Cost:</b> ${money(x.cost)} &nbsp; <b>Sell:</b> ${money(x.price)}</div><div><b>Min / Max:</b> ${Number(x.min_qty||0)} / ${Number(x.max_qty||0)} &nbsp; <b>Reorder:</b> ${Number(x.reorder_point??x.min_qty??0)}</div></div>${session?.role==='admin'?`<div class="card"><h3>Inventory Action</h3><div class="actions"><button onclick="partInventoryAction(${x.id},'receive')">Receive Stock</button><button class="secondary" onclick="partInventoryAction(${x.id},'adjust_add')">Adjust +</button><button class="secondary" onclick="partInventoryAction(${x.id},'adjust_remove')">Adjust −</button></div><div class="muted">Every stock change is written to the inventory ledger with user and timestamp.</div></div>`:''}</div><div><div class="barcodeBox"><b>ITTR Internal Barcode</b><img src="/api/parts/${x.id}/barcode.svg" alt="${esc(x.internal_barcode||'barcode')}"><div><b>${esc(x.internal_barcode||'')}</b></div><button class="secondary" onclick="printPartLabel(${x.id},'${encodeURIComponent(cleanImportedDisplayText(x.part_number)||'')}','${encodeURIComponent(cleanImportedDisplayText(x.description)||'')}','${encodeURIComponent(x.location||'')}' )">Print Label</button></div><div class="card"><b>Recognized Barcodes</b><div class="muted">${aliases.length?aliases.map(esc).join('<br>'):'No manufacturer aliases saved yet.'}</div>${session?.role==='admin'?`<button class="secondary" style="margin-top:8px" onclick="addPartBarcodeAlias(${x.id},'${encodeURIComponent(JSON.stringify(aliases))}')">+ Add Manufacturer Barcode</button>`:''}</div></div></div><div class="card"><h3>Inventory Ledger</h3>${tx.length?`<table class="partsTable"><thead><tr><th>Date</th><th>Movement</th><th>Qty</th><th>Reference</th><th>User</th></tr></thead><tbody>${tx.map(t=>`<tr><td>${esc(fmtDateTime(t.created_at))}</td><td>${esc(String(t.transaction_type||'').replaceAll('_',' '))}</td><td class="${Number(t.quantity_delta)>=0?'txPositive':'txNegative'}">${Number(t.quantity_delta)>0?'+':''}${Number(t.quantity_delta)}</td><td>${esc(t.reference||t.reason||'—')}${t.unit_number?`<div class="muted">Unit ${esc(t.unit_number)} · ${esc(t.task_name||'')}</div>`:''}</td><td>${esc(mechanicDisplay(t.username)||t.username||'')}</td></tr>`).join('')}</tbody></table>`:'<div class="muted">No inventory movements recorded yet.</div>'}</div>`}catch(e){body.innerHTML=`<div class="error">${esc(e.message||'Unable to open part.')}</div>`}}
function closePartDetail(){document.getElementById('partDetailModal')?.classList.remove('open');syncModalState()}
async function partInventoryAction(id,type){const qty=Number(prompt(type==='receive'?'Quantity received:':'Quantity:','1'));if(!qty||qty<=0)return;let reference='',reason='';if(type==='receive')reference=prompt('PO / vendor reference (optional):','')||'';else reason=prompt('Reason for adjustment:','Physical count adjustment')||'';try{await apiJSON(`/api/parts/${id}/transaction`,{method:'POST',body:JSON.stringify({type,qty,reference,reason,method:'parts_center'})});await openPartDetail(id);loadPartsCenter()}catch(e){alert(e.message||'Inventory update failed.') }}
async function addPartBarcodeAlias(id,encodedAliases){let aliases=[];try{aliases=JSON.parse(decodeURIComponent(encodedAliases))}catch{}const code=(prompt('Scan or enter manufacturer / vendor barcode:','')||'').trim();if(!code)return;if(aliases.some(x=>String(x).toLowerCase()===code.toLowerCase()))return alert('That barcode is already assigned to this part.');aliases.push(code);try{const d=await apiJSON(`/api/parts/${id}`,{method:'PATCH',body:JSON.stringify({barcodeAliases:aliases})});await openPartDetail(id)}catch(e){alert(e.message||'Unable to save barcode.')}}
function printPartLabel(id,pn,desc,loc){const w=window.open('','_blank','width=650,height=500');if(!w)return alert('Allow pop-ups to print labels.');const src=`/api/parts/${id}/barcode.svg`;w.document.write(`<html><head><title>ITTR Part Label</title><style>body{font-family:Arial;padding:20px}.label{width:3.5in;border:1px solid #bbb;padding:12px;text-align:center}.brand{font-weight:900;font-size:18px}.pn{font-size:18px;font-weight:800}.desc{font-size:12px;margin:4px 0}.loc{font-size:12px;font-weight:700}img{width:100%;height:90px}</style></head><body><div class="label"><div class="brand">IRON TEAM</div><div class="pn">${esc(decodeURIComponent(pn))}</div><div class="desc">${esc(decodeURIComponent(desc))}</div><img src="${src}"><div class="loc">${esc(decodeURIComponent(loc))}</div></div><script>setTimeout(()=>print(),700)<\/script></body></html>`);w.document.close()}
function openPartsScanner(mode='lookup',workOrderId=null,taskIndex=null){partsScannerContext={mode,workOrderId,taskIndex};const m=document.getElementById('partsScannerModal'),inp=document.getElementById('partsScanInput'),out=document.getElementById('partsScanResult');m?.classList.add('open');if(inp){inp.value='';setTimeout(()=>inp.focus(),80)}if(out)out.innerHTML='';syncModalState()}
...
async function verifyReleaseCompatibility(){
 try{
  ...
  const live=document.getElementById("onlineBetaBar");if(live)live.textContent=`ITTR v${expected} ONLINE · backend ${backend} · SMART WORKSPACE · CUSTOMER + VEHICLE HUB · FULLBAY · R2`;
  const b=document.getElementById("appErrorBanner");
  if(backend!==expected){
   if(b){
    b.style.display="block";
