...
function renderVehicleHistoryRows(){
 ...
 body.innerHTML=rows.length?rows.map(w=>`<tr><td><b>${esc(w.completedAt?fmtShortDate(w.completedAt):(cleanImportedDisplayText(w.date)||"—"))}</b></td><td><b>${w.source==="fullbay"?`SO #${esc(cleanVehicleMeta(w.id))}`:w.source==="invoice"?`Invoice ${esc(cleanVehicleMeta(w.invoice))}`:`WO #${esc(cleanVehicleMeta(w.id))}`}</b><div class="muted">${w.invoice?`Invoice #${esc(cleanVehicleMeta(w.invoice))}`:""}</div></td><td>${esc(vehicleHistorySummary(w))}</td><td>${w.mileage!=null?`${Number(w.mileage).toLocaleString()} mi`:"—"}</td><td>${['fullbay','invoice'].includes(w.source)?money(w.totalAmount||0):"—"}</td><td><button class="secondary" onclick="${w.source==="invoice"?`closeModal('vehicleProfileModal');showView('invoices');openInvoice(${Number(w.invoiceId||0)})`:w.source==="fullbay"?`openFullbayServiceOrder('${encodeURIComponent(String(w.id||""))}',${c.id||'null'},'${encodeURIComponent(String(w.unit||vehicleProfileCache.unit?.unit_number||""))}')`:`closeModal('vehicleProfileModal');openDetail(${Number(w.id)})`}">Open</button></td></tr>`).join(""):'<tr><td colspan="6" class="muted">No matching service orders.</td></tr>';
}
...
async function normalizeReceivingVendor(){
 ...
 try{
  ...
  if(hint)hint.textContent='Vendor will be reviewed when receiving.';
 }
}
...
/* ===== v24.6.5 PROFESSIONAL INVOICE OVERRIDES ===== */
let invoicePartSuggestTimer=null;
function invoiceDefaultLaborRate(){const i=activeInvoice?.invoice||{},first=(activeInvoice?.lines||[]).find(x=>x.line_type==='labor');return Number(i.default_labor_rate||first?.unit_price||0)}
function invoiceRatePreset(v){const n=Number(v);if(n===115)return'new';if(n===110)return'our';if(n===100)return'old';if(n===60)return'owner';return'custom'}
function invoiceRateControlHtml(labor,locked,idx){const v=Number(labor.unit_price||0),preset=invoiceRatePreset(v);return `<div class="invoiceRateControl"><select class="ilRatePreset" onchange="invoiceChooseRate(${labor.id},this.value,${idx})" ${locked?'disabled':''}><option value="new" ${preset==='new'?'selected':''}>New Client — $115</option><option value="our" ${preset==='our'?'selected':''}>Our Client — $110</option><option value="old" ${preset==='old'?'selected':''}>Old Client — $100</option><option value="owner" ${preset==='owner'?'selected':''}>Owner — $60</option><option value="custom" ${preset==='custom'?'selected':''}>Custom Rate</option></select><input class="ilPrice" type="number" step="0.01" min="0" value="${v}" oninput="invoiceCustomRate(${labor.id},${idx});invoiceLinePreview(${labor.id})" ${locked?'disabled':''}></div>`}
function invoiceChooseRate(id,key,idx){const rates={new:115,our:110,old:100,owner:60},row=document.querySelector(`[data-invoice-line="${id}"]`),inp=row?.querySelector('.ilPrice');if(!inp)return;if(key!=='custom')inp.value=rates[key];invoiceLinePreview(id);if(idx===0){activeInvoice.invoice.default_labor_rate=Number(inp.value||0);document.querySelectorAll('.invoiceOperation').forEach((op,j)=>{if(j>0){const line=op.querySelector('.invoiceLaborRow'),price=line?.querySelector('.ilPrice');if(price&&!line.dataset.rateTouched){price.value=inp.value;const sel=line.querySelector('.ilRatePreset');if(sel)sel.value=key;invoiceLinePreview(Number(line.dataset.invoiceLine))}}})}}
function invoiceCustomRate(id,idx){const row=document.querySelector(`[data-invoice-line="${id}"]`);if(!row)return;row.dataset.rateTouched='1';const sel=row.querySelector('.ilRatePreset');if(sel)sel.value=invoiceRatePreset(Number(row.querySelector('.ilPrice')?.value||0));if(idx===0)activeInvoice.invoice.default_labor_rate=Number(row.querySelector('.ilPrice')?.value||0)}
invoiceLaborOperationHtml=function(labor,children,locked,idx){return `<div class="invoiceOperation" data-invoice-operation="${labor.id}"><div class="invoiceOperationHead"><div class="invoiceOperationTitle"><span class="invoiceOperationBadge">LABOR ${idx+1}</span><span class="laborTitleText">${esc(labor.description||invoiceGroupName(labor)||'Service')}</span></div>${locked?'':`<div class="invoiceOperationActions"><button class="secondary" onclick="addInvoiceChildLine(${labor.id},'part')">+ Part</button><button class="secondary" onclick="addInvoiceChildLine(${labor.id},'sublet')">+ Sublet</button><button class="secondary" onclick="addInvoiceChildLine(${labor.id},'fee')">+ Fee</button></div>`}</div><div class="invoiceLaborRow" data-invoice-line="${labor.id}"><div class="invoiceLaborGrid"><div class="invoiceFieldMini descField"><label>LABOR / SERVICE PERFORMED</label><input class="ilDesc" value="${esc(labor.description||'')}" placeholder="Describe labor operation" ${locked?'disabled':''}></div><div class="invoiceFieldMini"><label>HOURS</label><input class="ilQty" type="number" step="0.01" min="0" value="${Number(labor.quantity||0)}" oninput="invoiceLinePreview(${labor.id})" ${locked?'disabled':''}></div><div class="invoiceFieldMini"><label>LABOR RATE</label>${invoiceRateControlHtml(labor,locked,idx)}</div><div class="invoiceFieldMini amountField"><label>LABOR AMOUNT</label><div class="invoiceAmountBox ilAmount">${invMoney(labor.line_total)}</div></div><label class="invoiceTaxCell"><input class="ilTax" type="checkbox" ${labor.taxable?'checked':''} ${locked?'disabled':''}> Tax</label>${locked?'':`<div class="invoiceDeleteCell"><button class="danger" onclick="deleteInvoiceLine(${labor.id})">×</button></div>`}</div><div class="invoiceLaborAdmin"><div class="invoiceFieldMini jobField"><label>Service / Job</label><input class="ilJob" value="${esc(invoiceGroupName(labor))}" ${locked?'disabled':''}></div><div class="invoiceFieldMini costField"><label>Internal Labor Cost</label><input class="ilCost" type="number" step="0.01" min="0" value="${Number(labor.unit_cost||0)}" ${locked?'disabled':''}></div>${locked?'':`<div class="invoiceLineActions"><button class="secondary" onclick="saveInvoiceLine(${labor.id})">Save Labor</button></div>`}</div><input type="hidden" class="ilType" value="labor"><input type="hidden" class="ilPart" value=""><input type="hidden" class="ilParent" value=""></div><div class="invoiceChildren">${children.length?`<div class="invoiceChildrenLabel">PARTS / CHARGES FOR THIS LABOR</div>${children.map(x=>invoiceChildLineHtml(x,locked,labor.id)).join('')}`:`<div class="muted" style="padding:10px 0">No parts or additional charges for this labor.</div>`}</div></div>`}
invoiceChildLineHtml=function(l,locked,parentId){const part=l.line_type==='part',typ=invoiceLineTypeLabel(l.line_type);return `<div class="invoiceChildRow" data-invoice-line="${l.id}"><div class="invoiceChildType">↳ ${esc(typ)}</div><div class="invoiceFieldMini ${part?'':'is-hidden'}"><label>PART #</label><input class="ilPart" value="${esc(l.part_number||'')}" placeholder="Search part #" oninput="scheduleInvoicePartSuggest(${l.id},'part')" autocomplete="off" ${locked?'disabled':''}></div><div class="invoiceFieldMini descField"><label>${part?'PART DESCRIPTION':'DESCRIPTION'}</label><input class="ilDesc" value="${esc(l.description||'')}" placeholder="${part?'Search inventory description':'Describe the charge'}" oninput="${part?`scheduleInvoicePartSuggest(${l.id},'desc')`:''}" autocomplete="off" ${locked?'disabled':''}></div><div class="invoiceFieldMini"><label>QTY</label><input class="ilQty" type="number" step="0.01" min="0" value="${Number(l.quantity||0)}" oninput="invoiceLinePreview(${l.id});invoiceSummaryPreview()" ${locked?'disabled':''}></div><div class="invoiceFieldMini"><label>${part?'UNIT PRICE':'RATE / PRICE'}</label><input class="ilPrice" type="number" step="0.01" min="0" value="${Number(l.unit_price||0)}" oninput="invoiceLinePreview(${l.id});invoiceSummaryPreview()" ${locked?'disabled':''}></div><div class="invoiceFieldMini amountField"><label>AMOUNT</label><div class="invoiceAmountBox ilAmount">${invMoney(l.line_total)}</div></div><label class="invoiceTaxCell"><input class="ilTax" type="checkbox" onchange="invoiceSummaryPreview()" ${l.taxable?'checked':''} ${locked?'disabled':''}> Tax</label>${locked?'':`<div class="invoiceDeleteCell"><button class="danger" onclick="deleteInvoiceLine(${l.id})">×</button></div>`}<div class="invoiceChildAdmin"><div class="invoiceFieldMini"><label>Internal Cost</label><input class="ilCost" type="number" step="0.01" min="0" value="${Number(l.unit_cost||0)}" ${locked?'disabled':''}></div>${locked?'':`<div class="invoiceLineActions"><button class="secondary" onclick="saveInvoiceLine(${l.id})">Save</button></div>`}</div><div class="invoicePartSuggest" id="invoicePartSuggest_${l.id}"></div><input type="hidden" class="ilType" value="${esc(l.line_type)}"><input type="hidden" class="ilJob" value="${esc(invoiceGroupName(l))}"><input type="hidden" class="ilParent" value="${parentId||l.parent_line_id||''}"></div>`}
function scheduleInvoicePartSuggest(id,field){clearTimeout(invoicePartSuggestTimer);invoicePartSuggestTimer=setTimeout(()=>runInvoicePartSuggest(id,field),160)}
async function runInvoicePartSuggest(id,field){const row=document.querySelector(`[data-invoice-line="${id}"]`),box=document.getElementById(`invoicePartSuggest_${id}`);if(!row||!box)return;const q=String((field==='part'?row.querySelector('.ilPart'):row.querySelector('.ilDesc'))?.value||'').trim();if(q.length<2){box.innerHTML='';return}try{const d=await apiJSON(`/api/parts?q=${encodeURIComponent(q)}&limit=12`);box.innerHTML=(d.items||[]).map(x=>`<button type="button" class="invoicePartOption" onclick="chooseInvoiceInventoryPart(${id},${Number(x.id)},'${encodeURIComponent(String(x.part_number||''))}','${encodeURIComponent(String(x.description||''))}',${Number(x.price||0)},${Number(x.cost||0)},${x.sell_taxable!==false})"><b>${esc(cleanPartField(x.part_number))}</b><span class="pdesc">${esc(cleanPartField(x.description))}</span><small class="stock">${Number(x.available||0)} avail.</small><small class="price">${invMoney(x.price||0)}</small></button>`).join('')||'<div class="muted" style="padding:10px">No inventory matches.</div>'}catch(e){box.innerHTML=`<div class="error" style="padding:10px">${esc(e.message)}</div>`}}
function chooseInvoiceInventoryPart(lineId,partId,pn,desc,price,cost,taxable){const row=document.querySelector(`[data-invoice-line="${lineId}"]`);if(!row)return;row.querySelector('.ilPart').value=decodeURIComponent(pn);row.querySelector('.ilDesc').value=decodeURIComponent(desc);row.querySelector('.ilPrice').value=Number(price||0);row.querySelector('.ilCost').value=Number(cost||0);row.querySelector('.ilTax').checked=Boolean(taxable);row.dataset.inventoryPartId=String(partId);document.getElementById(`invoicePartSuggest_${lineId}`).innerHTML='';invoiceLinePreview(lineId);invoiceSummaryPreview()}
addInvoiceChildLine=async function(parentLineId,type='part'){const i=activeInvoice?.invoice;if(!i)return;try{await saveAllInvoiceLines();const parent=activeInvoice.lines.find(x=>String(x.id)===String(parentLineId)),job=String(document.querySelector(`[data-invoice-line="${parentLineId}"] .ilJob`)?.value||parent?.job_name||'Service');const r=await fetch(`/api/invoices/${i.id}/lines`,{method:'POST',headers:authHeaders({'Content-Type':'application/json'}),body:JSON.stringify({lineType:type,description:'',quantity:1,unitPrice:0,taxable:type==='part',jobName:job,parentLineId})}),d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to add charge');await openInvoice(i.id);setTimeout(()=>{const row=document.querySelector(`[data-invoice-line="${d.id}"]`);row?.querySelector(type==='part'?'.ilPart':'.ilDesc')?.focus();row?.scrollIntoView({behavior:'smooth',block:'center'})},40)}catch(e){alert(e.message)}}
addInvoiceLine=async function(type='labor',jobName='Service',parentLineId=null){const i=activeInvoice?.invoice;if(!i)return;const typ=parentLineId?type:'labor';try{await saveAllInvoiceLines();const rate=typ==='labor'?invoiceDefaultLaborRate():0;const r=await fetch(`/api/invoices/${i.id}/lines`,{method:'POST',headers:authHeaders({'Content-Type':'application/json'}),body:JSON.stringify({lineType:typ,description:typ==='labor'?jobName:'',quantity:1,unitPrice:rate,taxable:typ==='part',jobName:jobName||'Service',parentLineId})}),d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to add line');await openInvoice(i.id);setTimeout(()=>document.querySelector(`[data-invoice-line="${d.id}"] .ilDesc`)?.focus(),40)}catch(e){alert(e.message)}}
function invoiceSummaryPreview(){if(!activeInvoice)return;let linesSubtotal=0,taxable=0;document.querySelectorAll('[data-invoice-line]').forEach(row=>{const q=Number(row.querySelector('.ilQty')?.value||0),p=Number(row.querySelector('.ilPrice')?.value||0),amt=Math.max(0,q*p);linesSubtotal+=amt;if(row.querySelector('.ilTax')?.checked)taxable+=amt});const supplies=Number(document.getElementById('invShopSupplies')?.value||0),env=Number(document.getElementById('invEnvFee')?.value||0),discount=Math.max(0,Number(document.getElementById('invDiscount')?.value||0)),rate=Number(document.getElementById('invTaxRate')?.value||0),exempt=Boolean(document.getElementById('invTaxExempt')?.checked),subtotal=Math.max(0,linesSubtotal+supplies+env-discount),tax=exempt?0:Math.max(0,taxable-Math.min(discount,taxable))*rate/100,total=subtotal+tax,paid=Number(activeInvoice.invoice.amount_paid||0),bal=Math.max(0,total-paid);const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=invMoney(v)};set('invLiveSubtotal',subtotal);set('invLiveTax',tax);set('invLiveTotal',total);set('invLiveBalance',bal)}
async function lookupIllinoisTax(){const city=String(document.getElementById('invTaxCity')?.value||'').trim(),county=String(document.getElementById('invTaxCounty')?.value||'').trim(),note=document.getElementById('invTaxSourceNote');if(!city){if(note){note.className='taxSourceNote warn';note.textContent='Enter the Illinois tax location city first.'}return}try{if(note){note.className='taxSourceNote';note.textContent='Checking Illinois Department of Revenue…'}const d=await apiJSON(`/api/tax/illinois-rate?city=${encodeURIComponent(city)}&county=${encodeURIComponent(county)}`);if(d.ok&&d.rate!=null){document.getElementById('invTaxRate').value=Number(d.rate);activeInvoice.invoice.tax_rate_source=d.source;note.className='taxSourceNote ok';note.textContent=`Official IDOR jurisdiction rate: ${Number(d.rate).toFixed(3)}% · effective ${d.record?.effectiveDate||''}`;invoiceSummaryPreview()}else{note.className='taxSourceNote warn';note.innerHTML=`${esc(d.message||'Address-specific lookup required.')} <a href="https://mytax.illinois.gov/" target="_blank" rel="noopener">Open official Illinois Tax Rate Finder ↗</a>`}}catch(e){note.className='taxSourceNote warn';note.innerHTML=`${esc(e.message)} <a href="https://mytax.illinois.gov/" target="_blank" rel="noopener">Open official Illinois Tax Rate Finder ↗</a>`}}
async function reopenInvoice(id){const reason=prompt('Reason for reopening this finalized/paid invoice (required):');if(!reason)return;const r=await fetch(`/api/invoices/${id}/reopen`,{method:'POST',headers:authHeaders({'Content-Type':'application/json'}),body:JSON.stringify({reason})}),d=await r.json();if(!r.ok)return alert(d.error||'Unable to reopen');await openInvoice(id);await loadInvoices()}
async function deleteDraftInvoice(id){if(!confirm('Permanently delete this unpaid DRAFT invoice? This cannot be undone.'))return;const r=await fetch(`/api/invoices/${id}/permanent`,{method:'DELETE',headers:authHeaders()}),d=await r.json();if(!r.ok)return alert(d.error||'Unable to delete invoice');closeModal('invoiceEditorModal');await loadInvoices()}
renderInvoiceEditor=function(){const {invoice:i,lines,payments}=activeInvoice,locked=['paid','void'].includes(i.status),firstLabor=lines.find(x=>x.line_type==='labor');if(!Number(i.default_labor_rate||0)&&firstLabor)i.default_labor_rate=Number(firstLabor.unit_price||0);document.getElementById('invoiceEditor').innerHTML=`<div class="toolbar"><div><h1 style="margin:0">${esc(i.invoice_number)}</h1><div class="muted">${esc(i.customer_name)} · Unit ${esc(i.unit_number||'—')} ${i.work_order_id?`· WO #${esc(i.work_order_id)}`:''}</div></div><span class="invoiceStatus is-${i.status}">${esc(i.status)}</span></div><div class="invoiceEditorGrid"><div><div class="card"><h2 style="margin-top:0">Customer & Vehicle</h2><div class="row"><div class="field"><label>Customer</label><input id="invCustomer" value="${esc(i.customer_name)}" ${locked?'disabled':''}></div><div class="field"><label>Unit</label><input id="invUnit" value="${esc(i.unit_number||'')}" ${locked?'disabled':''}></div></div><div class="row"><div class="field"><label>VIN</label><input id="invVin" value="${esc(i.vin||'')}" ${locked?'disabled':''}></div><div class="field"><label>Mileage</label><input id="invMileage" type="number" value="${Number(i.mileage||0)||''}" ${locked?'disabled':''}></div><div class="field"><label>Customer PO #</label><input id="invPo" value="${esc(i.po_number||'')}" ${locked?'disabled':''}></div></div><div class="row"><div class="field"><label>Invoice Date</label><input id="invDate" type="date" value="${esc(String(i.invoice_date||'').slice(0,10))}" ${locked?'disabled':''}></div><div class="field"><label>Due Date</label><input id="invDue" type="date" value="${esc(String(i.due_date||'').slice(0,10))}" ${locked?'disabled':''}></div><div class="field"><label>Terms</label><input id="invTerms" value="${esc(i.terms||'')}" ${locked?'disabled':''}></div></div></div><div class="card"><div class="toolbar"><div><h2 style="margin:0">Labor, Parts & Charges</h2><div class="muted">Each labor operation owns its parts. New labor automatically starts with the invoice's first/default labor rate.</div></div>${locked?'':`<button class="secondary" onclick="toggleInvoiceAddMenu()">+ New Labor</button>`}</div>${locked?'':`<div id="invoiceAddMenu" class="invoiceAddMenu is-hidden"><div class="field"><label>New Labor / Service</label><input id="invoiceNewJobName" placeholder="PM Service, Replace Wheel Seal, Diagnostics"></div><button class="invoiceAddType newLaborOnly" onclick="addInvoiceLineFromMenu('labor')">+ Create Labor</button></div>`}${invoiceGroupedLinesHtml(lines,locked)}</div><div class="card"><h2 style="margin-top:0">Customer Note</h2><textarea id="invCustomerNote" rows="3" ${locked?'disabled':''}>${esc(i.customer_note||'')}</textarea><h3>Internal Note</h3><textarea id="invInternalNote" rows="2" ${locked?'disabled':''}>${esc(i.internal_note||'')}</textarea></div></div><div><div class="card invoiceTotals"><h2 style="margin-top:0">Invoice Summary</h2><div class="invoiceTaxTools"><label class="invoiceTaxExempt"><input id="invTaxExempt" type="checkbox" ${i.tax_exempt?'checked':''} onchange="invoiceSummaryPreview()" ${locked?'disabled':''}> Tax Exempt Invoice</label><div class="field"><label>Exemption / E-Number</label><input id="invTaxExemptNumber" value="${esc(i.tax_exempt_number||'')}" placeholder="E-number / certificate" ${locked?'disabled':''}></div><div class="field"><label>Exemption Reason</label><input id="invTaxExemptReason" value="${esc(i.tax_exempt_reason||'')}" placeholder="Government, exempt organization, resale, etc." ${locked?'disabled':''}></div></div><div class="field"><label>Illinois Tax Location City</label><input id="invTaxCity" value="${esc(i.tax_location_city||'')}" placeholder="e.g. Matteson" ${locked?'disabled':''}></div><div class="field"><label>County (recommended)</label><input id="invTaxCounty" value="${esc(i.tax_location_county||'')}" placeholder="e.g. Cook" ${locked?'disabled':''}></div><div class="field"><label>Sales Tax %</label><input id="invTaxRate" type="number" step="0.001" value="${Number(i.tax_rate||0)}" oninput="invoiceSummaryPreview()" ${locked?'disabled':''}></div>${locked?'':`<button class="secondary" style="width:100%;margin-bottom:8px" onclick="lookupIllinoisTax()">Get Current Illinois Rate</button>`}<div id="invTaxSourceNote" class="taxSourceNote">${esc(i.tax_rate_source||'Tax is applied only to taxable lines. Illinois combined rates can vary by location.')}</div><div class="field"><label>Shop Supplies</label><input id="invShopSupplies" type="number" step="0.01" value="${Number(i.shop_supplies||0)}" oninput="invoiceSummaryPreview()" ${locked?'disabled':''}></div><div class="field"><label>Environmental / Other Fee</label><input id="invEnvFee" type="number" step="0.01" value="${Number(i.environmental_fee||0)}" oninput="invoiceSummaryPreview()" ${locked?'disabled':''}></div><div class="field"><label>Invoice Discount</label><input id="invDiscount" type="number" step="0.01" value="${Number(i.discount||0)}" oninput="invoiceSummaryPreview()" ${locked?'disabled':''}></div><hr><div class="invoiceSummaryLive"><div class="muted">Subtotal <b id="invLiveSubtotal" style="float:right">${invMoney(i.subtotal)}</b></div><div class="muted">Tax <b id="invLiveTax" style="float:right">${invMoney(i.tax)}</b></div><div style="margin-top:12px">Total <b id="invLiveTotal" style="float:right">${invMoney(i.total)}</b></div><div class="muted">Paid <b style="float:right">${invMoney(i.amount_paid)}</b></div><div style="margin-top:12px">Balance Due</div><div class="moneyBig" id="invLiveBalance">${invMoney(i.balance_due)}</div></div><div class="actions" style="margin-top:14px">${locked&&i.status==='paid'?`<button class="secondary" onclick="reopenInvoice(${i.id})">Reopen for Correction</button>`:''}${!locked?`<button onclick="saveInvoiceHeader()">Save Changes</button>`:''}<button class="secondary" onclick="downloadInvoicePDF(${i.id})">PDF</button>${i.status==='draft'?`<button class="success" onclick="finalizeInvoice(${i.id})">Finalize / Mark Sent</button>`:''}${['sent','partial'].includes(i.status)?`<button class="success" onclick="recordInvoicePayment(${i.id},${Number(i.balance_due||0)})">Record Payment</button>`:''}${i.status!=='void'&&Number(i.amount_paid||0)===0?`<button class="danger" onclick="voidInvoice(${i.id})">Void</button>`:''}</div><h3>Payments</h3>${payments.map(p=>`<div class="historyitem"><b>${invMoney(p.amount)}</b> · ${esc(p.method||'Other')}<div class="muted">${esc(String(p.paid_at||'').slice(0,19).replace('T',' '))} ${p.reference?`· ${esc(p.reference)}`:''}</div></div>`).join('')||'<div class="muted">No payments recorded.</div>'}<div class="invoiceDangerZone">${i.status==='draft'&&Number(i.amount_paid||0)===0?`<button class="danger" onclick="deleteDraftInvoice(${i.id})">Delete Draft Invoice</button>`:'<div class="muted">Finalized invoices are voided instead of permanently deleted to preserve accounting history.</div>'}</div><div class="invoiceFooterPolicy">PDF includes manufacturer parts-warranty responsibility language and the 50-mile wheel/tire torque recheck notice.</div></div></div></div>`;invoiceSummaryPreview()}
saveInvoiceHeader=async function(){const i=activeInvoice.invoice;try{await saveAllInvoiceLines();const b={customerName:document.getElementById('invCustomer').value,unitNumber:document.getElementById('invUnit').value,vin:document.getElementById('invVin').value,poNumber:document.getElementById('invPo').value,mileage:Number(document.getElementById('invMileage')?.value||0),invoiceDate:document.getElementById('invDate').value,dueDate:document.getElementById('invDue').value,terms:document.getElementById('invTerms').value,taxRate:Number(document.getElementById('invTaxRate').value||0),defaultLaborRate:invoiceDefaultLaborRate(),taxExempt:Boolean(document.getElementById('invTaxExempt')?.checked),taxExemptNumber:document.getElementById('invTaxExemptNumber')?.value||'',taxExemptReason:document.getElementById('invTaxExemptReason')?.value||'',taxRateSource:activeInvoice.invoice.tax_rate_source||'',taxLocationCity:document.getElementById('invTaxCity')?.value||'',taxLocationCounty:document.getElementById('invTaxCounty')?.value||'',shopSupplies:Number(document.getElementById('invShopSupplies').value||0),environmentalFee:Number(document.getElementById('invEnvFee').value||0),discount:Number(document.getElementById('invDiscount').value||0),customerNote:document.getElementById('invCustomerNote').value,internalNote:document.getElementById('invInternalNote').value};const r=await fetch(`/api/invoices/${i.id}`,{method:'PUT',headers:authHeaders({'Content-Type':'application/json'}),body:JSON.stringify(b)}),d=await r.json();if(!r.ok)throw new Error(d.error||'Save failed');await openInvoice(i.id);await loadInvoices();return true}catch(e){alert(e.message);return false}}
