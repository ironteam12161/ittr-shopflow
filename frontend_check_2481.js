...
function allowedViewForRole(view){
 ...
 return ["admin","manager"].includes(session.role) ? adminViews.includes(view) : mechanicViews.includes(view);
}
...
function toggleShopAiVoice(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition,status=document.getElementById('shopAiVoiceStatus');if(!SR){if(status)status.textContent='Voice input is not supported by this browser.';return}if(shopAiVoiceRecognition){shopAiVoiceRecognition.stop();shopAiVoiceRecognition=null;return}const r=new SR();shopAiVoiceRecognition=r;r.continuous=false;r.interimResults=true;r.lang=currentLanguage==='uk'?'uk-UA':'en-US';const input=document.getElementById('shopAiInput');if(status)status.textContent='Listening…';r.onresult=e=>{let t='';for(let i=e.resultIndex;i<e.results.length;i++)t+=e.results[i][0].transcript;if(input)input.value=t};r.onerror=()=>{if(status)status.textContent='Voice stopped.'};r.onend=()=>{shopAiVoiceRecognition=null;if(status)status.textContent=''};r.start()}
