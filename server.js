...
app.get("/api/admin/customer-crm-diagnostics",auth,adminOnly,async(req,res)=>{
 const out={ok:false,version:"24.6.1",tables:{},columns:{},counts:{},sync:null,error:""};
 try{
  const db=requireDb();
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.6.1",backend:"24.6.1",build:"ITTR-24.6.1-SERVICE-ORDER-SMART-INVOICE-20260912"}));
app.get("/api/health",async(req,res)=>{let db=false;try{if(pool){await pool.query("SELECT 1");db=true}}catch{}res.json({ok:true,db,aiConfigured:Boolean(openRouterClient||client),aiProvider:openRouterClient?"openrouter":client?"openai":"none",version:"24.6.1",photoStorageConfigured:r2Configured})});
...
app.post('/api/invoices/:id/lines',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,b=req.body||{},inv=(await db.query('SELECT status FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(!['draft','sent','partial'].includes(inv.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is locked.'})}const typ=['labor','part','fee','sublet','other'].includes(b.lineType)?b.lineType:'other';const q=await db.query(`INSERT INTO customer_invoice_lines(invoice_id,sort_order,job_uid,job_name,line_type,description,part_number,quantity,unit_price,unit_cost,taxable,discount,line_total) VALUES($1::bigint,(SELECT coalesce(max(sort_order),0)+1 FROM customer_invoice_lines WHERE invoice_id=$1::bigint),$2,$3,$4,$5,$6,$7::numeric,$8::numeric,$9::numeric,$10::boolean,$11::numeric,greatest(0,$7::numeric*$8::numeric-$11::numeric)) RETURNING id`,[id,String(b.jobUid||''),String(b.jobName||''),typ,String(b.description||'Line item'),String(b.partNumber||''),Number(b.quantity||1),Number(b.unitPrice||0),Number(b.unitCost||0),b.taxable===true,Number(b.discount||0)]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true,id:q.rows[0].id})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.put('/api/invoices/:id/lines/:lineId',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const b=req.body||{},id=req.params.id;const inv=(await db.query('SELECT status FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv||!['draft','sent','partial'].includes(inv.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is locked or missing.'})}await db.query(`UPDATE customer_invoice_lines SET job_name=$3,line_type=$4,description=$5,part_number=$6,quantity=$7::numeric,unit_price=$8::numeric,unit_cost=$9::numeric,taxable=$10::boolean,discount=$11::numeric,line_total=greatest(0,$7::numeric*$8::numeric-$11::numeric) WHERE id=$2::bigint AND invoice_id=$1::bigint`,[id,req.params.lineId,String(b.jobName||''),['labor','part','fee','sublet','other'].includes(b.lineType)?b.lineType:'other',String(b.description||''),String(b.partNumber||''),Number(b.quantity||0),Number(b.unitPrice||0),Number(b.unitCost||0),b.taxable===true,Number(b.discount||0)]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.delete('/api/invoices/:id/lines/:lineId',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const inv=(await db.query('SELECT status FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[req.params.id])).rows[0];if(!inv||!['draft','sent','partial'].includes(inv.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is locked or missing.'})}await db.query('DELETE FROM customer_invoice_lines WHERE id=$1::bigint AND invoice_id=$2::bigint',[req.params.lineId,req.params.id]);await recalcInvoice(db,req.params.id);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
