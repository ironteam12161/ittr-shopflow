...
app.get("/api/admin/customer-crm-diagnostics",auth,adminOnly,async(req,res)=>{
 const out={ok:false,version:"24.24.2",tables:{},columns:{},counts:{},sync:null,error:""};
 try{
  const db=requireDb();
...
// ===== v24.24.2: Samsara + duplicate customer management =====
...
app.get('/api/samsara/integration-status',auth,managerPermission('customers'),async(req,res)=>{res.json({ok:true,build:'24.24.2',configured:!!String(process.env.SAMSARA_API_TOKEN||'').trim(),unitSync:true})});
...
// v24.24.2 — safe cleanup of incomplete imported Fullbay service history only.
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.24.2",backend:"24.24.2",build:"ITTR-24.24.2-PRINT-LEGAL-PART-LOOKUP-20260914"}));
app.get("/api/health",async(req,res)=>{let db=false;try{if(pool){await pool.query("SELECT 1");db=true}}catch{}res.json({ok:true,db,aiConfigured:Boolean(openRouterClient||client),aiProvider:openRouterClient?"openrouter":client?"openai":"none",version:"24.24.2",photoStorageConfigured:r2Configured})});
