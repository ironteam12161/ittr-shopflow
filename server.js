...
app.get("/api/customers/:id/profile",auth,adminOnly,async(req,res)=>{
 ...
 try{
  ...
  let invoiceHistory=[];try{const ir=await db.query(`SELECT id,invoice_number,unit_number,vin,mileage,invoice_date,status,total,balance_due FROM customer_invoices WHERE status<>'void' AND customer_id=$1::bigint ORDER BY invoice_date DESC,id DESC LIMIT 500`,[customer.id]);invoiceHistory=ir.rows.map(x=>({source:'invoice',id:x.id,invoice:x.invoice_number,unit:x.unit_number,vin:x.vin,completedAt:x.invoice_date,status:x.status,mileage:x.mileage,totalAmount:Number(x.total||0),balanceDue:Number(x.balance_due||0),tasks:[{t:`Invoice ${x.invoice_number}`}]}))}catch(e){warnings.push('ITTR invoice history unavailable.');console.error('Customer invoice history warning:',e?.message)}
  const history=[...invoiceHistory,...ittrHistory,...fullbayHistory].sort((a,b)=>new Date(b.completedAt||b.date||0)-new Date(a.completedAt||a.date||0)).slice(0,1200);
...
app.get("/api/customer-units/:id/profile",auth,adminOnly,async(req,res,next)=>{try{
 ...
 let invoiceHistory=[];try{const ir=await db.query(`SELECT id,invoice_number,unit_number,vin,mileage,invoice_date,status,total,balance_due FROM customer_invoices WHERE status<>'void' AND (unit_id=$1::bigint OR (lower(coalesce(unit_number,''))=lower($2) AND lower(coalesce(vin,''))=lower($3))) ORDER BY invoice_date DESC,id DESC LIMIT 500`,[unit.id,unit.unit_number||'',unit.vin||'']);invoiceHistory=ir.rows.map(x=>({source:'invoice',id:x.id,invoice:x.invoice_number,unit:x.unit_number,vin:x.vin,completedAt:x.invoice_date,status:x.status,mileage:x.mileage,totalAmount:Number(x.total||0),balanceDue:Number(x.balance_due||0),tasks:[{t:`Invoice ${x.invoice_number}`}]}))}catch(e){console.error('Vehicle invoice history warning:',e?.message)}
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.7.0",backend:"24.7.0",build:"ITTR-24.7.0-FULLBAY-STYLE-INVOICE-LINES-20260912"}));
...
async function recalcInvoice(db,id){
 const lr=await db.query(`SELECT coalesce(sum(greatest(0,quantity*unit_price-discount)),0) subtotal,coalesce(sum(CASE WHEN taxable THEN greatest(0,quantity*unit_price-discount) ELSE 0 END),0) taxable FROM customer_invoice_lines WHERE invoice_id=$1::bigint`,[id]);
 const inv=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv)return null;
 const linesSubtotal=invoiceMoney(lr.rows[0].subtotal),lineTaxable=invoiceMoney(lr.rows[0].taxable),shop=invoiceMoney(inv.shop_supplies),env=invoiceMoney(inv.environmental_fee),fees=shop+env,discount=Math.max(0,invoiceMoney(inv.discount)),subtotal=Math.max(0,linesSubtotal+fees-discount);
 // Illinois service shops commonly include shop/environmental supply charges in the tax base when taxable property is transferred. Invoice-level exemption overrides all tax.
 const taxableBeforeDiscount=lineTaxable+shop+env,taxable=Math.max(0,taxableBeforeDiscount-Math.min(discount,taxableBeforeDiscount));
 const tax=inv.tax_exempt?0:invoiceMoney(taxable*invoiceMoney(inv.tax_rate)/100),total=invoiceMoney(subtotal+tax),paid=invoiceMoney(inv.amount_paid),balance=invoiceMoney(Math.max(0,total-paid));
 let status=inv.status;if(status!=='void'&&status!=='draft'){status=balance<=0&&total>0?'paid':paid>0?'partial':status==='paid'?'sent':status}
 await db.query(`UPDATE customer_invoices SET subtotal=$2::numeric,taxable_subtotal=$3::numeric,tax=$4::numeric,total=$5::numeric,balance_due=$6::numeric,status=$7::text,paid_at=CASE WHEN $7::text='paid' THEN coalesce(paid_at,now()) ELSE NULL END,updated_at=now() WHERE id=$1::bigint`,[id,subtotal,taxable,tax,total,balance,status]);
 return (await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint',[id])).rows[0]
}
...
app.post('/api/invoices',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const b=req.body||{},num=await nextInvoiceNumber(db);const q=await db.query(`INSERT INTO customer_invoices(invoice_number,customer_id,customer_name,unit_id,unit_number,vin,mileage,po_number,invoice_date,due_date,terms,tax_rate,customer_note,internal_note,created_by) VALUES($1,$2::bigint,$3,$4::bigint,$5,$6,$7::numeric,$8,coalesce($9::date,CURRENT_DATE),coalesce($10::date,CURRENT_DATE),$11,$12::numeric,$13,$14,$15) RETURNING id`,[num,b.customerId||null,String(b.customerName||'Customer'),b.unitId||null,String(b.unitNumber||''),String(b.vin||''),Number(b.mileage||0)||null,String(b.poNumber||''),b.invoiceDate||null,b.dueDate||null,String(b.terms||'Due on Receipt'),Number(b.taxRate||0),String(b.customerNote||''),String(b.internalNote||''),req.user.username]);await recalcInvoice(db,q.rows[0].id);await db.query('COMMIT');res.json({ok:true,id:q.rows[0].id})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.put('/api/invoices/:id',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,cur=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!cur){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(cur.status==='void'){await db.query('ROLLBACK');return res.status(409).json({error:'Void invoices are locked. Reopen or create a replacement invoice.'})}const b=req.body||{};await db.query(`UPDATE customer_invoices SET customer_name=$2,unit_number=$3,vin=$4,po_number=$5,mileage=$15::numeric,invoice_date=$6::date,due_date=$7::date,terms=$8,tax_rate=$9::numeric,shop_supplies=$10::numeric,environmental_fee=$11::numeric,discount=$12::numeric,customer_note=$13,internal_note=$14,tax_exempt=$16::boolean,tax_exempt_reason=$17,tax_exempt_certificate=$18,updated_at=now() WHERE id=$1::bigint`,[id,String(b.customerName??cur.customer_name),String(b.unitNumber??cur.unit_number??''),String(b.vin??cur.vin??''),String(b.poNumber??cur.po_number??''),b.invoiceDate||cur.invoice_date,b.dueDate||cur.due_date,String(b.terms??cur.terms??''),Number(b.taxRate??cur.tax_rate??0),Number(b.shopSupplies??cur.shop_supplies??0),Number(b.environmentalFee??cur.environmental_fee??0),Number(b.discount??cur.discount??0),String(b.customerNote??cur.customer_note??''),String(b.internalNote??cur.internal_note??''),Number(b.mileage??cur.mileage??0)||null,Boolean(b.taxExempt??cur.tax_exempt),String(b.taxExemptReason??cur.tax_exempt_reason??''),String(b.taxExemptCertificate??cur.tax_exempt_certificate??'')]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.post('/api/invoices/:id/lines',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,b=req.body||{},inv=(await db.query('SELECT status FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(!['draft','sent','partial','paid'].includes(inv.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is locked.'})}const typ=['labor','part','fee','sublet','other'].includes(b.lineType)?b.lineType:'other';let parentLineId=b.parentLineId?Number(b.parentLineId):null,jobUid=String(b.jobUid||''),jobName=String(b.jobName||'');if(parentLineId){const p=(await db.query(`SELECT id,job_uid,job_name FROM customer_invoice_lines WHERE id=$1::bigint AND invoice_id=$2::bigint AND line_type='labor'`,[parentLineId,id])).rows[0];if(!p){await db.query('ROLLBACK');return res.status(409).json({error:'Parent labor operation not found.'})}jobUid=String(p.job_uid||'');jobName=String(p.job_name||'');if(typ==='labor')parentLineId=null}const q=await db.query(`INSERT INTO customer_invoice_lines(invoice_id,sort_order,job_uid,job_name,line_type,description,part_number,quantity,unit_price,unit_cost,taxable,discount,line_total,parent_line_id) VALUES($1::bigint,(SELECT coalesce(max(sort_order),0)+1 FROM customer_invoice_lines WHERE invoice_id=$1::bigint),$2,$3,$4,$5,$6,$7::numeric,$8::numeric,$9::numeric,$10::boolean,$11::numeric,greatest(0,$7::numeric*$8::numeric-$11::numeric),$12::bigint) RETURNING id`,[id,jobUid,jobName,typ,String(b.description??''),String(b.partNumber||''),Number(b.quantity||1),Number(b.unitPrice||0),Number(b.unitCost||0),b.taxable===true,Number(b.discount||0),parentLineId]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true,id:q.rows[0].id})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
...
app.post('/api/invoices/:id/reopen',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,reason=String(req.body?.reason||'').trim();if(!reason)return res.status(400).json({error:'Reason for reopening is required.'});const cur=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!cur){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(cur.status==='void'){await db.query('ROLLBACK');return res.status(409).json({error:'Void invoice cannot be reopened. Create a replacement invoice.'})}await db.query(`UPDATE customer_invoices SET status='draft',reopened_at=now(),reopened_by=$2,reopen_reason=$3,updated_at=now() WHERE id=$1::bigint`,[id,req.user.username,reason]);await recalcInvoice(db,id);await db.query('COMMIT');await audit(req.user.username,'invoice_reopened',{invoiceId:id,reason});res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.delete('/api/invoices/:id',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,cur=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!cur){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}const pc=Number((await db.query('SELECT count(*)::int n FROM customer_invoice_payments WHERE invoice_id=$1::bigint',[id])).rows[0]?.n||0);if(pc>0){await db.query('ROLLBACK');return res.status(409).json({error:'Invoices with payments cannot be permanently deleted. Void/refund it to preserve accounting history.'})}if(!['draft','void'].includes(cur.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Finalize invoices must be voided before permanent deletion.'})}await db.query('DELETE FROM customer_invoice_lines WHERE invoice_id=$1::bigint',[id]);await db.query('DELETE FROM customer_invoices WHERE id=$1::bigint',[id]);await db.query('COMMIT');await audit(req.user.username,'invoice_deleted',{invoiceId:id,invoiceNumber:cur.invoice_number});res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.get('/api/invoices/:id/pdf',auth,adminOnly,async(req,res,next)=>{try{const db=requireDb(),x=await getInvoiceBundle(db,req.params.id);if(!x)return res.status(404).json({error:'Invoice not found.'});const i=x.invoice;res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`attachment; filename="${String(i.invoice_number).replace(/[^A-Za-z0-9_-]/g,'_')}.pdf"`);res.setHeader('Cache-Control','private, no-store');const doc=new PDFDocument({size:'LETTER',margin:40,bufferPages:true,info:{Title:`Invoice ${i.invoice_number}`}});doc.pipe(res);const money=n=>`$${invoiceMoney(n).toFixed(2)}`,L=40,W=532,navy='#0f1b33',blue='#1d4ed8',gray='#64748b',line='#dbe3ee';
 const logoPath=path.join(process.cwd(),'assets','iron-team-logo.png');try{if(fs.existsSync(logoPath))doc.image(logoPath,L,32,{fit:[58,58]})}catch{}
 doc.fillColor(navy).font('Helvetica-Bold').fontSize(17).text('IRON TEAM TRUCK & TRAILER',108,38,{width:290});doc.fontSize(17).text('REPAIR',108,59);doc.fillColor(gray).font('Helvetica').fontSize(8.5).text('Heavy-Duty Truck & Trailer Service',108,82);
 doc.fillColor(navy).font('Helvetica-Bold').fontSize(25).text('INVOICE',390,38,{width:182,align:'right'});doc.fillColor(blue).fontSize(10).text(i.invoice_number,390,72,{width:182,align:'right'});doc.fillColor(gray).font('Helvetica').fontSize(8).text(String(i.status||'').toUpperCase(),390,88,{width:182,align:'right'});
 let y=118;doc.roundedRect(L,y,W,86,7).fillAndStroke('#f8fafc',line);doc.fillColor(navy).font('Helvetica-Bold').fontSize(8).text('BILL TO',L+14,y+12);doc.fontSize(11).text(i.customer_name,L+14,y+28,{width:270});doc.fillColor(gray).font('Helvetica').fontSize(8.5).text(`Unit ${i.unit_number||'—'}  ·  VIN ${i.vin||'—'}${i.mileage?`  ·  ${Number(i.mileage).toLocaleString()} mi`:''}`,L+14,y+48,{width:300});doc.fillColor(navy).font('Helvetica-Bold').fontSize(8).text('DATE',365,y+13);doc.font('Helvetica').text(String(i.invoice_date||'').slice(0,10),435,y+13);doc.font('Helvetica-Bold').text('DUE',365,y+31);doc.font('Helvetica').text(String(i.due_date||'').slice(0,10),435,y+31);doc.font('Helvetica-Bold').text('PO #',365,y+49);doc.font('Helvetica').text(i.po_number||'—',435,y+49);doc.font('Helvetica-Bold').text('TERMS',365,y+67);doc.font('Helvetica').text(i.terms||'—',435,y+67);y+=108;
 const header=()=>{doc.rect(L,y,W,24).fill(navy);doc.fillColor('#fff').font('Helvetica-Bold').fontSize(7.5).text('SERVICE / DESCRIPTION',L+10,y+8,{width:300});doc.text('QTY/HRS',382,y+8,{width:52,align:'right'});doc.text('RATE/PRICE',438,y+8,{width:62,align:'right'});doc.text('AMOUNT',505,y+8,{width:58,align:'right'});y+=30};header();
 const labors=x.lines.filter(l=>l.line_type==='labor'),children=x.lines.filter(l=>l.line_type!=='labor'),used=new Set();for(const labor of labors){if(y>680){doc.addPage();y=46;header()}doc.fillColor(navy).font('Helvetica-Bold').fontSize(9).text(labor.description||labor.job_name||'Labor',L+10,y,{width:305});doc.font('Helvetica').fontSize(8.5).text(Number(labor.quantity||0).toFixed(2),382,y,{width:52,align:'right'});doc.text(money(labor.unit_price),438,y,{width:62,align:'right'});doc.font('Helvetica-Bold').text(money(labor.line_total),505,y,{width:58,align:'right'});y+=19;for(const c of children.filter(z=>String(z.parent_line_id||'')===String(labor.id))){used.add(c.id);if(y>700){doc.addPage();y=46;header()}doc.fillColor(gray).font('Helvetica-Bold').fontSize(7.5).text(c.line_type==='part'?`PART ${c.part_number||''}`:String(c.line_type).toUpperCase(),L+20,y,{width:125});doc.fillColor(navy).font('Helvetica').fontSize(8).text(c.description||'',L+145,y,{width:225});doc.text(String(Number(c.quantity||0)),382,y,{width:52,align:'right'});doc.text(money(c.unit_price),438,y,{width:62,align:'right'});doc.text(money(c.line_total),505,y,{width:58,align:'right'});y+=18}doc.moveTo(L,y+1).lineTo(L+W,y+1).strokeColor(line).stroke();y+=10}
 for(const c of children.filter(z=>!used.has(z.id))){doc.fillColor(gray).font('Helvetica-Bold').fontSize(7.5).text(String(c.line_type).toUpperCase(),L+20,y,{width:125});doc.fillColor(navy).font('Helvetica').fontSize(8).text(c.description||'',L+145,y,{width:225});doc.text(String(Number(c.quantity||0)),382,y,{width:52,align:'right'});doc.text(money(c.unit_price),438,y,{width:62,align:'right'});doc.text(money(c.line_total),505,y,{width:58,align:'right'});y+=18}
 y=Math.max(y+16,520);if(y>650){doc.addPage();y=60}const rx=370;doc.fillColor(gray).font('Helvetica').fontSize(9).text('Labor / Parts Subtotal',rx,y);doc.fillColor(navy).text(money(invoiceMoney(i.subtotal)-invoiceMoney(i.shop_supplies)-invoiceMoney(i.environmental_fee)+invoiceMoney(i.discount)),485,y,{width:78,align:'right'});y+=18;if(invoiceMoney(i.shop_supplies)){doc.fillColor(gray).text('Shop Supplies',rx,y);doc.fillColor(navy).text(money(i.shop_supplies),485,y,{width:78,align:'right'});y+=18}if(invoiceMoney(i.environmental_fee)){doc.fillColor(gray).text('Environmental / Other Fee',rx,y);doc.fillColor(navy).text(money(i.environmental_fee),485,y,{width:78,align:'right'});y+=18}if(invoiceMoney(i.discount)){doc.fillColor(gray).text('Discount',rx,y);doc.fillColor(navy).text(`-${money(i.discount)}`,485,y,{width:78,align:'right'});y+=18}doc.fillColor(gray).text(i.tax_exempt?'Sales Tax — EXEMPT':`Sales Tax (${Number(i.tax_rate||0).toFixed(3)}%)`,rx,y);doc.fillColor(navy).text(money(i.tax),485,y,{width:78,align:'right'});y+=22;doc.moveTo(rx,y-5).lineTo(L+W,y-5).strokeColor(navy).stroke();doc.font('Helvetica-Bold').fontSize(13).text('TOTAL',rx,y);doc.text(money(i.total),485,y,{width:78,align:'right'});y+=23;doc.fontSize(11).text('BALANCE DUE',rx,y);doc.text(money(i.balance_due),485,y,{width:78,align:'right'});
 let ny=y+42;if(i.tax_exempt){doc.fillColor(navy).font('Helvetica-Bold').fontSize(8).text(`TAX EXEMPT${i.tax_exempt_certificate?` · Certificate ${i.tax_exempt_certificate}`:''}`,L,ny,{width:300});ny+=14}if(i.customer_note){doc.fillColor(navy).font('Helvetica-Bold').fontSize(8).text('CUSTOMER NOTE',L,ny);doc.fillColor(gray).font('Helvetica').fontSize(8).text(i.customer_note,L,ny+13,{width:310});ny+=42}
 const warranty='PARTS WARRANTY: Parts are warranted by the manufacturer under the manufacturer’s applicable warranty terms. Iron Team Truck & Trailer Repair does not provide or extend the manufacturer’s warranty.';const tire='TIRE / WHEEL SERVICE NOTICE: After tire or wheel installation/service, the customer must have wheel fasteners checked and re-torqued after approximately 50 miles of operation and periodically thereafter as required by the wheel/tire manufacturer or fleet procedure.';
 if(ny>690){doc.addPage();ny=70}doc.fillColor(navy).font('Helvetica-Bold').fontSize(8).text('IMPORTANT SERVICE TERMS',L,ny);doc.fillColor(gray).font('Helvetica').fontSize(7.5).text(warranty,L,ny+14,{width:W});doc.text(tire,L,ny+42,{width:W});
 const pages=doc.bufferedPageRange();for(let n=0;n<pages.count;n++){doc.switchToPage(n);doc.fillColor('#94a3b8').font('Helvetica').fontSize(7).text(`Thank you for choosing Iron Team Truck & Trailer Repair  ·  ${i.invoice_number}  ·  Page ${n+1} of ${pages.count}`,L,742,{width:W,align:'center',lineBreak:false})}doc.end()}catch(e){next(e)}});
