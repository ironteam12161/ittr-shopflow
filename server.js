...
app.post('/api/invoices',auth,managerPermission("invoices"),async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const b=req.body||{},num=await nextInvoiceNumber(db);const q=await db.query(`INSERT INTO customer_invoices(invoice_number,customer_id,customer_name,unit_id,unit_number,vin,mileage,po_number,invoice_date,due_date,terms,tax_rate,customer_note,internal_note,created_by) VALUES($1,$2::bigint,$3,$4::bigint,$5,$6,$7::numeric,$8,coalesce($9::date,CURRENT_DATE),coalesce($10::date,CURRENT_DATE),$11,$12::numeric,$13,$14,$15) RETURNING id`,[num,b.customerId||null,String(b.customerName||'Customer'),b.unitId||null,String(b.unitNumber||''),String(b.vin||''),Number(b.mileage||0)||null,String(b.poNumber||''),b.invoiceDate||null,b.dueDate||null,String(b.terms||'Due on Receipt'),Number(b.taxRate||0),String(b.customerNote||''),String(b.internalNote||''),req.user.username]);await recalcInvoice(db,q.rows[0].id);await db.query('COMMIT');res.json({ok:true,id:q.rows[0].id})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
