...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.6.5",backend:"24.6.5",build:"ITTR-24.6.5-PRO-INVOICE-SYNC-TAX-20260914"}));
...
app.get('/api/invoices/:id/pdf',auth,adminOnly,async(req,res,next)=>{try{const db=requireDb(),x=await getInvoiceBundle(db,req.params.id);if(!x)return res.status(404).json({error:'Invoice not found.'});const i=x.invoice;res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`attachment; filename="${String(i.invoice_number).replace(/[^A-Za-z0-9_-]/g,'_')}.pdf"`);res.setHeader('Cache-Control','private, no-store');const doc=new PDFDocument({size:'LETTER',margin:38,bufferPages:true,info:{Title:`Invoice ${i.invoice_number}`}});doc.pipe(res);const money=n=>`$${invoiceMoney(n).toFixed(2)}`,L=38,W=536,logoPath=path.join(__dirname,'public','assets','iron-team-logo.png');try{if(fs.existsSync(logoPath))doc.image(logoPath,L,34,{fit:[58,58]})}catch{}doc.fillColor('#111827').font('Helvetica-Bold').fontSize(17).text('IRON TEAM TRUCK & TRAILER REPAIR',L+68,38,{width:300});doc.font('Helvetica').fontSize(8.5).fillColor('#64748b').text('Heavy-Duty Truck & Trailer Service',L+68,61);doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(25).text('INVOICE',390,36,{width:184,align:'right'});doc.fontSize(10).fillColor('#2563eb').text(i.invoice_number,390,68,{width:184,align:'right'});let y=104;doc.roundedRect(L,y,W,64,6).fillAndStroke('#f8fafc','#dbe3ec');doc.fillColor('#334155').font('Helvetica-Bold').fontSize(8).text('BILL TO',L+12,y+10);doc.fillColor('#0f172a').fontSize(11).text(i.customer_name,L+12,y+24,{width:250});doc.font('Helvetica').fontSize(8.5).fillColor('#475569').text(`Unit ${i.unit_number||'—'}  ·  VIN ${i.vin||'—'}${i.mileage?`  ·  ${Number(i.mileage).toLocaleString()} mi`:''}`,L+12,y+43,{width:330});doc.font('Helvetica-Bold').fontSize(8).fillColor('#334155').text('DATE',390,y+10);doc.font('Helvetica').text(String(i.invoice_date||'').slice(0,10),450,y+10);doc.font('Helvetica-Bold').text('DUE',390,y+26);doc.font('Helvetica').text(String(i.due_date||'').slice(0,10),450,y+26);doc.font('Helvetica-Bold').text('PO #',390,y+42);doc.font('Helvetica').text(i.po_number||'—',450,y+42);y+=82;const header=()=>{doc.rect(L,y,W,22).fill('#0f172a');doc.fillColor('#fff').font('Helvetica-Bold').fontSize(7.5).text('SERVICE / DESCRIPTION',L+8,y+7,{width:300});doc.text('QTY/HRS',360,y+7,{width:50,align:'right'});doc.text('RATE/PRICE',414,y+7,{width:70,align:'right'});doc.text('AMOUNT',490,y+7,{width:76,align:'right'});y+=28};header();const labors=x.lines.filter(l=>l.line_type==='labor'),children=x.lines.filter(l=>l.line_type!=='labor');for(const labor of labors){if(y>680){doc.addPage();y=44;header()}doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(9).text(`LABOR  ${labor.description||labor.job_name||'Service'}`,L+8,y,{width:300});doc.font('Helvetica').text(Number(labor.quantity||0).toFixed(2),360,y,{width:50,align:'right'});doc.text(money(labor.unit_price),414,y,{width:70,align:'right'});doc.font('Helvetica-Bold').text(money(labor.line_total),490,y,{width:76,align:'right'});y+=18;for(const c of children.filter(z=>String(z.parent_line_id||'')===String(labor.id))){if(y>700){doc.addPage();y=44;header()}const label=c.line_type==='part'?`PART ${c.part_number||''}`:String(c.line_type||'CHARGE').toUpperCase();doc.fillColor('#475569').font('Helvetica-Bold').fontSize(8).text(label,L+20,y,{width:105});doc.font('Helvetica').fillColor('#111827').text(c.description||'',L+128,y,{width:178});doc.text(String(Number(c.quantity||0)),360,y,{width:50,align:'right'});doc.text(money(c.unit_price),414,y,{width:70,align:'right'});doc.text(money(c.line_total),490,y,{width:76,align:'right'});y+=17}doc.moveTo(L,y+1).lineTo(L+W,y+1).strokeColor('#e2e8f0').stroke();y+=10}y=Math.max(y+8,470);const right=386;doc.fillColor('#334155').font('Helvetica').fontSize(9).text('Subtotal',right,y);doc.text(money(i.subtotal),480,y,{width:86,align:'right'});y+=17;if(i.tax_exempt){doc.font('Helvetica-Bold').fillColor('#166534').text(`Tax Exempt${i.tax_exempt_number?` · ${i.tax_exempt_number}`:''}`,right,y,{width:180});doc.text('$0.00',480,y,{width:86,align:'right'})}else{doc.fillColor('#334155').font('Helvetica').text(`Sales Tax (${Number(i.tax_rate||0).toFixed(3)}%)`,right,y);doc.text(money(i.tax),480,y,{width:86,align:'right'})}y+=20;doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(13).text('TOTAL',right,y);doc.text(money(i.total),480,y,{width:86,align:'right'});y+=22;doc.fontSize(10).text('BALANCE DUE',right,y);doc.text(money(i.balance_due),480,y,{width:86,align:'right'});if(i.customer_note){y+=34;doc.fillColor('#334155').font('Helvetica-Bold').fontSize(8).text('CUSTOMER NOTE',L,y);doc.font('Helvetica').fontSize(8.5).fillColor('#111827').text(i.customer_note,L,y+13,{width:320})}const footerNote='PARTS WARRANTY: Parts warranties, if any, are provided by the part manufacturer. Iron Team Truck & Trailer Repair does not independently warrant manufacturer defects except as required by law. WHEEL/TIRE SERVICE: After tire or wheel replacement/removal, customer must have wheel fastener torque rechecked after 50 miles of operation.';const pages=doc.bufferedPageRange();for(let n=0;n<pages.count;n++){doc.switchToPage(n);doc.moveTo(L,720).lineTo(L+W,720).strokeColor('#cbd5e1').stroke();doc.font('Helvetica').fontSize(6.8).fillColor('#64748b').text(footerNote,L,728,{width:W,align:'left',lineGap:1});doc.text(`${i.invoice_number} · Page ${n+1} of ${pages.count}`,L,766,{width:W,align:'center'})}doc.end()}catch(e){next(e)}});
