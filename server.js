...
app.get("/api/admin/customer-crm-diagnostics",auth,adminOnly,async(req,res)=>{
 const out={ok:false,version:"24.6.0",tables:{},columns:{},counts:{},sync:null,error:""};
 try{
  const db=requireDb();
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.6.0",backend:"24.6.0",build:"ITTR-24.6.0-PRO-INVOICING-AR-PAYMENTS-20260912"}));
app.get("/api/health",async(req,res)=>{let db=false;try{if(pool){await pool.query("SELECT 1");db=true}}catch{}res.json({ok:true,db,aiConfigured:Boolean(openRouterClient||client),aiProvider:openRouterClient?"openrouter":client?"openai":"none",version:"24.6.0",photoStorageConfigured:r2Configured})});
...
app.post('/api/invoices',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const b=req.body||{},num=await nextInvoiceNumber(db);const q=await db.query(`INSERT INTO customer_invoices(invoice_number,customer_id,customer_name,unit_number,vin,po_number,invoice_date,due_date,terms,tax_rate,customer_note,internal_note,created_by) VALUES($1,$2::bigint,$3,$4,$5,$6,coalesce($7::date,CURRENT_DATE),coalesce($8::date,CURRENT_DATE),$9,$10::numeric,$11,$12,$13) RETURNING id`,[num,b.customerId||null,String(b.customerName||'Customer'),String(b.unitNumber||''),String(b.vin||''),String(b.poNumber||''),b.invoiceDate||null,b.dueDate||null,String(b.terms||'Due on Receipt'),Number(b.taxRate||0),String(b.customerNote||''),String(b.internalNote||''),req.user.username]);await recalcInvoice(db,q.rows[0].id);await db.query('COMMIT');res.json({ok:true,id:q.rows[0].id})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.put('/api/invoices/:id',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,cur=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!cur){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(!['draft','sent','partial'].includes(cur.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Paid or void invoices are locked.'})}const b=req.body||{};await db.query(`UPDATE customer_invoices SET customer_name=$2,unit_number=$3,vin=$4,po_number=$5,invoice_date=$6::date,due_date=$7::date,terms=$8,tax_rate=$9::numeric,shop_supplies=$10::numeric,environmental_fee=$11::numeric,discount=$12::numeric,customer_note=$13,internal_note=$14,updated_at=now() WHERE id=$1::bigint`,[id,String(b.customerName??cur.customer_name),String(b.unitNumber??cur.unit_number??''),String(b.vin??cur.vin??''),String(b.poNumber??cur.po_number??''),b.invoiceDate||cur.invoice_date,b.dueDate||cur.due_date,String(b.terms??cur.terms??''),Number(b.taxRate??cur.tax_rate??0),Number(b.shopSupplies??cur.shop_supplies??0),Number(b.environmentalFee??cur.environmental_fee??0),Number(b.discount??cur.discount??0),String(b.customerNote??cur.customer_note??''),String(b.internalNote??cur.internal_note??'')]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
