...
app.get("/api/customer-units/:id/profile",auth,adminOnly,async(req,res,next)=>{try{
 ...
 try{
  const ir=await db.query(`SELECT i.id,i.invoice_number,i.status,i.invoice_date,i.created_at,i.unit_number,i.vin,i.mileage,i.total,i.work_order_id,i.service_order_id,
    coalesce(json_agg(json_build_object('id',l.id,'type',l.line_type,'description',l.description,'partNumber',l.part_number,'jobName',l.job_name,'quantity',l.quantity,'unitPrice',l.unit_price,'lineTotal',l.line_total) ORDER BY l.sort_order,l.id) FILTER (WHERE l.id IS NOT NULL),'[]'::json) lines
  ...
  invoiceHistory=ir.rows.map(x=>({source:"invoice",id:x.id,invoice:x.invoice_number,unit:x.unit_number,vin:x.vin,completedAt:x.invoice_date||x.created_at,status:x.status,mileage:x.mileage,totalAmount:Number(x.total||0),workOrderId:x.work_order_id,serviceOrderId:x.service_order_id,tasks:(Array.isArray(x.lines)?x.lines:[]).map(l=>({t:[l.jobName,l.description,l.partNumber].filter(Boolean).join(" · "),lineType:l.type,partNumber:l.partNumber,quantity:l.quantity,unitPrice:l.unitPrice,lineTotal:l.lineTotal}))}))
