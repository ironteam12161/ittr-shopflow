...
app.get("/api/admin/customer-crm-diagnostics",auth,adminOnly,async(req,res)=>{
 const out={ok:false,version:"24.20.0",tables:{},columns:{},counts:{},sync:null,error:""};
 try{
  const db=requireDb();
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.20.0",backend:"24.20.0",build:"ITTR-24.20.0-PRINT-LEGAL-PART-LOOKUP-20260914"}));
app.get("/api/health",async(req,res)=>{let db=false;try{if(pool){await pool.query("SELECT 1");db=true}}catch{}res.json({ok:true,db,aiConfigured:Boolean(openRouterClient||client),aiProvider:openRouterClient?"openrouter":client?"openai":"none",version:"24.20.0",photoStorageConfigured:r2Configured})});
...
async function enrichLegacyInvoiceDraft(db,draft){
 ...
 const partMatches=[];for(const svc of Array.isArray(d.services)?d.services:[]){for(const part of Array.isArray(svc.parts)?svc.parts:[]){let m=null;const token=String(part.partNumber||part.description||'').trim();if(token){const q=await db.query(`SELECT id,part_number,description,buy_price,sell_price,quantity,location FROM fullbay_import_parts WHERE lower(coalesce(part_number,''))=lower($1) OR lower(coalesce(description,''))=lower($1) OR lower(coalesce(part_number,'')) LIKE '%'||lower($1)||'%' OR lower(coalesce(description,'')) LIKE '%'||lower($1)||'%' ORDER BY CASE WHEN lower(coalesce(part_number,''))=lower($1) THEN 0 ELSE 1 END,id LIMIT 1`,[token]);m=q.rows[0]||null}partMatches.push({source:part.partNumber||part.description||'',match:m})}}
