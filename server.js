...
async function initDb(){
 ...
 ALTER TABLE customer_invoices ADD COLUMN IF NOT EXISTS shop_supplies_taxable BOOLEAN DEFAULT FALSE;
 ALTER TABLE customer_invoices ADD COLUMN IF NOT EXISTS environmental_fee_taxable BOOLEAN DEFAULT FALSE;
 ALTER TABLE customer_invoices ADD COLUMN IF NOT EXISTS tax_source TEXT;
 ALTER TABLE customer_invoices ADD COLUMN IF NOT EXISTS tax_effective_date DATE;
...
app.get("/api/admin/customer-crm-diagnostics",auth,adminOnly,async(req,res)=>{
 const out={ok:false,version:"24.6.6",tables:{},columns:{},counts:{},sync:null,error:""};
 try{
  const db=requireDb();
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.6.6",backend:"24.6.6",build:"ITTR-24.6.6-INVOICE-RATES-PARTS-ILLINOIS-TAX-20260912"}));
app.get("/api/health",async(req,res)=>{let db=false;try{if(pool){await pool.query("SELECT 1");db=true}}catch{}res.json({ok:true,db,aiConfigured:Boolean(openRouterClient||client),aiProvider:openRouterClient?"openrouter":client?"openai":"none",version:"24.6.6",photoStorageConfigured:r2Configured})});
...
async function recalcInvoice(db,id){const lr=await db.query(`SELECT coalesce(sum(greatest(0,quantity*unit_price-discount)),0) subtotal,coalesce(sum(CASE WHEN taxable THEN greatest(0,quantity*unit_price-discount) ELSE 0 END),0) taxable FROM customer_invoice_lines WHERE invoice_id=$1::bigint`,[id]);const inv=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv)return null;const linesSubtotal=invoiceMoney(lr.rows[0].subtotal),lineTaxable=invoiceMoney(lr.rows[0].taxable),shop=invoiceMoney(inv.shop_supplies),env=invoiceMoney(inv.environmental_fee),fees=shop+env,feeTaxable=(inv.shop_supplies_taxable?shop:0)+(inv.environmental_fee_taxable?env:0),gross=Math.max(0,linesSubtotal+fees),taxableGross=Math.max(0,lineTaxable+feeTaxable),discount=Math.min(gross,Math.max(0,invoiceMoney(inv.discount))),subtotal=Math.max(0,gross-discount),taxableDiscount=gross>0?Math.min(taxableGross,discount*(taxableGross/gross)):0,tax=invoiceMoney(Math.max(0,taxableGross-taxableDiscount)*invoiceMoney(inv.tax_rate)/100),total=invoiceMoney(subtotal+tax),paid=invoiceMoney(inv.amount_paid),balance=invoiceMoney(Math.max(0,total-paid));let status=inv.status;if(status!=='void'&&status!=='draft'){status=balance<=0&&total>0?'paid':paid>0?'partial':status==='paid'?'sent':status}await db.query(`UPDATE customer_invoices SET subtotal=$2::numeric,taxable_subtotal=$3::numeric,tax=$4::numeric,total=$5::numeric,balance_due=$6::numeric,status=$7::text,paid_at=CASE WHEN $7::text='paid' THEN coalesce(paid_at,now()) ELSE NULL END,updated_at=now() WHERE id=$1::bigint`,[id,subtotal,taxableGross,tax,total,balance,status]);return (await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint',[id])).rows[0]}
async function getInvoiceBundle(db,id){const inv=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint',[id])).rows[0];if(!inv)return null;const lines=(await db.query('SELECT * FROM customer_invoice_lines WHERE invoice_id=$1::bigint ORDER BY sort_order,id',[id])).rows;const payments=(await db.query('SELECT * FROM customer_invoice_payments WHERE invoice_id=$1::bigint ORDER BY paid_at,id',[id])).rows;let customerDefaults={defaultLaborRate:0};if(inv.customer_id){const c=(await db.query('SELECT default_labor_rate,credit_terms,taxable,tax_exempt_number FROM fullbay_import_customers WHERE id=$1::bigint',[inv.customer_id])).rows[0];if(c)customerDefaults={defaultLaborRate:Number(c.default_labor_rate||0),creditTerms:c.credit_terms||'',taxable:c.taxable,taxExemptNumber:c.tax_exempt_number||''}}return {invoice:inv,lines,payments,customerDefaults}}
const ILLINOIS_IDOR_RATE_URL='https://tax.illinois.gov/content/dam/soi/en/web/tax/research/taxrates/documents/salestaxrates/ordmache-current.txt';
let illinoisRateFileCache={at:0,text:''};
async function getIllinoisRateFile(){if(illinoisRateFileCache.text&&Date.now()-illinoisRateFileCache.at<6*60*60*1000)return illinoisRateFileCache.text;const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),8000);try{const r=await fetch(ILLINOIS_IDOR_RATE_URL,{headers:{'User-Agent':'ITTR-ShopFlow/24.6.6 tax-rate-check'},signal:controller.signal});if(!r.ok)throw new Error(`IDOR rate file returned HTTP ${r.status}`);const text=await r.text();if(!text||text.length<10000)throw new Error('IDOR rate file was empty or incomplete');illinoisRateFileCache={at:Date.now(),text};return text}finally{clearTimeout(timer)}}
function parseIllinoisRateLine(line){if(String(line||'').length<74)return null;const locationId=line.slice(0,10).trim(),city=line.slice(10,35).trim(),county=line.slice(35,60).trim(),addressOverride=line.slice(60,61),effectiveDate=line.slice(61,69),rateCode=line.slice(69,74),rate=Number(rateCode)/1000;if(!locationId||!city||!Number.isFinite(rate))return null;return {locationId,city,county,addressOverride,effectiveDate,rate}}
async function currentIllinoisTaxRate(city,county){const text=await getIllinoisRateFile(),wantCity=String(city||'').trim().toUpperCase(),wantCounty=String(county||'').trim().toUpperCase();let matches=[];for(const line of text.split(/\r?\n/)){const x=parseIllinoisRateLine(line);if(!x)continue;if(x.city.toUpperCase()!==wantCity)continue;if(wantCounty&&x.county.toUpperCase()!==wantCounty)continue;matches.push(x)}if(!matches.length)throw new Error(`Official IDOR rate not found for ${city}${county?`, ${county} County`:''}.`);const x=matches[0];let warning='';if(x.addressOverride==='Y'&&x.rate===0)warning='IDOR marks this municipality as address-specific. Use the official address Tax Rate Finder before finalizing.';else if(x.addressOverride==='Y')warning='IDOR indicates an address override may apply in part of this jurisdiction. Verify the shop address if it is inside a special district.';return {...x,source:ILLINOIS_IDOR_RATE_URL,official:true,warning}}
app.get('/api/tax/illinois/current',auth,adminOnly,async(req,res,next)=>{try{const city=String(req.query.city||process.env.SHOP_TAX_CITY||'Matteson').trim(),county=String(req.query.county||process.env.SHOP_TAX_COUNTY||'Cook').trim();const x=await currentIllinoisTaxRate(city,county);res.setHeader('Cache-Control','private, max-age=1800');res.json(x)}catch(e){next(e)}});
...
app.put('/api/invoices/:id',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,cur=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!cur){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(!['draft','sent','partial'].includes(cur.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Paid or void invoices are locked.'})}const b=req.body||{};await db.query(`UPDATE customer_invoices SET customer_name=$2,unit_number=$3,vin=$4,po_number=$5,mileage=$15::numeric,invoice_date=$6::date,due_date=$7::date,terms=$8,tax_rate=$9::numeric,shop_supplies=$10::numeric,environmental_fee=$11::numeric,discount=$12::numeric,customer_note=$13,internal_note=$14,shop_supplies_taxable=$16::boolean,environmental_fee_taxable=$17::boolean,tax_source=$18,tax_effective_date=coalesce(nullif($19::text,'')::date,tax_effective_date),updated_at=now() WHERE id=$1::bigint`,[id,String(b.customerName??cur.customer_name),String(b.unitNumber??cur.unit_number??''),String(b.vin??cur.vin??''),String(b.poNumber??cur.po_number??''),b.invoiceDate||cur.invoice_date,b.dueDate||cur.due_date,String(b.terms??cur.terms??''),Number(b.taxRate??cur.tax_rate??0),Number(b.shopSupplies??cur.shop_supplies??0),Number(b.environmentalFee??cur.environmental_fee??0),Number(b.discount??cur.discount??0),String(b.customerNote??cur.customer_note??''),String(b.internalNote??cur.internal_note??''),Number(b.mileage??cur.mileage??0)||null,Boolean(b.shopSuppliesTaxable??cur.shop_supplies_taxable),Boolean(b.environmentalFeeTaxable??cur.environmental_fee_taxable),String(b.taxSource??cur.tax_source??''),String(b.taxEffectiveDate||'')]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
