...
app.get("/api/customer-units/:id/profile",auth,adminOnly,async(req,res,next)=>{try{
 ...
 let invoiceHistory=[];
 try{
  const ir=await db.query(`SELECT i.id,i.invoice_number,i.status,i.invoice_date,i.created_at,i.unit_number,i.vin,i.mileage,i.total,i.work_order_id,i.service_order_id,
    coalesce(json_agg(json_build_object('id',l.id,'type',l.line_type,'description',l.description,'partNumber',l.part_number,'jobName',l.job_name,'quantity',l.quantity,'unitPrice',l.unit_price,'lineTotal',l.line_total,'tech',null) ORDER BY l.sort_order,l.id) FILTER (WHERE l.id IS NOT NULL),'[]'::json) lines
  ...
  invoiceHistory=ir.rows.map(x=>({source:"invoice",id:x.id,invoice:x.invoice_number,unit:x.unit_number,vin:x.vin,completedAt:x.invoice_date||x.created_at,status:x.status,mileage:x.mileage,totalAmount:Number(x.total||0),workOrderId:x.work_order_id,serviceOrderId:x.service_order_id,tasks:(Array.isArray(x.lines)?x.lines:[]).map(l=>({t:[l.jobName,l.description].filter(Boolean).join(" · "),description:l.description,jobName:l.jobName,lineType:l.type,partNumber:l.partNumber,quantity:l.quantity,unitPrice:l.unitPrice,lineTotal:l.lineTotal}))}))
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.8.0",backend:"24.8.0",build:"ITTR-24.8.0-AI-RBAC-PAYMENTS-COMMS-20260914"}));
...
app.get("/api/admin/staff",auth,adminOnly,async(req,res,next)=>{try{const q=await requireDb().query("SELECT username,display_name,role,email,permissions,language,active,created_at FROM auth_users ORDER BY role,display_name");res.json({items:q.rows.map(publicUser)})}catch(e){next(e)}});
...
app.put('/api/invoices/:id',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,cur=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!cur){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(!['draft','sent','partial'].includes(cur.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Paid or void invoices are locked.'})}const b=req.body||{};await db.query(`UPDATE customer_invoices SET customer_name=$2,unit_number=$3,vin=$4,po_number=$5,mileage=$15::numeric,invoice_date=$6::date,due_date=$7::date,terms=$8,tax_rate=$9::numeric,shop_supplies=$10::numeric,environmental_fee=$11::numeric,discount=$12::numeric,customer_note=$13,internal_note=$14,updated_at=now() WHERE id=$1::bigint`,[id,String(b.customerName??cur.customer_name),String(b.unitNumber??cur.unit_number??''),String(b.vin??cur.vin??''),String(b.poNumber??cur.po_number??''),b.invoiceDate||cur.invoice_date,b.dueDate||cur.due_date,String(b.terms??cur.terms??''),Number(b.taxRate??cur.tax_rate??0),Number(b.shopSupplies??cur.shop_supplies??0),Number(b.environmentalFee??cur.environmental_fee??0),Number(b.discount??cur.discount??0),String(b.customerNote??cur.customer_note??''),String(b.internalNote??cur.internal_note??''),Number(b.mileage??cur.mileage??0)||null]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
...
app.post("/api/ai/shop-chat",auth,async(req,res)=>{try{const question=String(req.body?.message||'').trim();if(!question)return res.status(400).json({error:'message required'});if(question.length>3000)return res.status(400).json({error:'Message is too long.'});const db=requireDb();const unitMatch=question.match(/\b([A-HJ-NPR-Z0-9]{17})\b/i)||question.match(/\b(\d{2,8})\b/)||question.match(/(?:unit|truck|тра[кк]|машин|тягач|номер)\s*#?\s*([A-Za-z0-9-]{2,20})/i);let unit=null,history=[],invoices=[],work=[];if(unitMatch){const token=unitMatch[1];const uq=await db.query(`SELECT * FROM customer_units WHERE lower(coalesce(unit_number,''))=lower($1) OR lower(coalesce(vin,''))=lower($1) ORDER BY updated_at DESC LIMIT 1`,[token]);unit=uq.rows[0]||null;if(unit){const keys=[String(unit.unit_number||''),String(unit.vin||'')].filter(Boolean);const hr=await db.query(`SELECT service_order,invoice_number,actual_correction,complaint,action_completed_at,hours,unit_miles,component,system,tech FROM fullbay_service_history WHERE lower(coalesce(unit_number,''))=lower($1) OR lower(coalesce(vin,''))=lower($2) ORDER BY action_completed_at DESC NULLS LAST LIMIT 120`,[keys[0]||'',keys[1]||'']);history=hr.rows;const ir=await db.query(`SELECT i.id,i.invoice_number,i.invoice_date,i.mileage,i.status,l.line_type,l.job_name,l.description,l.part_number,l.quantity FROM customer_invoices i LEFT JOIN customer_invoice_lines l ON l.invoice_id=i.id WHERE i.unit_id=$1 OR lower(coalesce(i.unit_number,''))=lower($2) OR lower(coalesce(i.vin,''))=lower($3) ORDER BY i.invoice_date DESC,l.sort_order,l.id LIMIT 150`,[unit.id,unit.unit_number||'',unit.vin||'']);invoices=ir.rows;}}
const financialAllowed=req.user.role==='admin'||(req.user.role==='manager'&&req.user.permissions?.financials!==false);const context={unit:unit?{unitNumber:unit.unit_number,vin:unit.vin,year:unit.year,make:unit.make,model:unit.model,mileage:unit.mileage}:null,fullbayHistory:history.map(x=>({...x})),ittrInvoiceHistory:invoices.map(x=>financialAllowed?x:{...x,status:undefined})};const system=`You are ITTR Shop Assistant for a heavy-duty truck and trailer repair shop. Answer in the same language as the user's question unless they ask otherwise. Use ONLY the supplied shop database context for questions about a specific unit, service history, dates, mileage, repairs or parts. Never invent a repair, date, mileage, diagnosis, price or part. If the database context does not establish the answer, say that clearly. For questions like oil change, search semantically across PM service, oil/filter change, lubrication, engine oil, actual correction and complaint. Prefer the most recent completed matching job and include date, mileage and service order/invoice when available. Mechanics may ask in Ukrainian, Polish, Spanish, Russian or English. Do not reveal internal costs or customer financial data unless it exists in context.`;res.json({result:await textAI(system,`Question: ${question}\n\nSHOP DATABASE CONTEXT:\n${JSON.stringify(context)}`),matchedUnit:unit?unit.unit_number:null})}catch(e){return aiErrorResponse(res,e,'Shop AI failed')}});
