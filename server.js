...
function getSamsaraToken(){
 ...
 // Railway values are sometimes pasted with quotes or an accidental `Bearer ` prefix.
 // Store/send only the raw token; never return it from diagnostics or logs.
 if((token.startsWith('\"')&&token.endsWith('\"'))||(token.startsWith("'")&&token.endsWith("'")))token=token.slice(1,-1).trim();
 token=token.replace(/^Bearer\s+/i,'').trim();
 return token;
}
function samsaraTokenPresent(){return !!getSamsaraToken()}
async function samsaraRequest(pathname,query={}){
 const token=getSamsaraToken();
...
app.get('/api/samsara/status',auth,managerPermission('customers'),async(req,res)=>{
 const configured=samsaraTokenPresent();
