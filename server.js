...
async function ensureInvoiceNotOverpaid(db,id){const x=(await db.query('SELECT total,amount_paid FROM customer_invoices WHERE id=$1::bigint',[id])).rows[0];if(x&&invoiceMoney(x.amount_paid)>invoiceMoney(x.total)+0.009){const e=new Error('Edit would make the invoice total lower than payments already recorded. Reconcile the payment first.');e.status=409;throw e}return x}
