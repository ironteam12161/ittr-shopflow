...
async function initDb(){
 ...
 CREATE INDEX IF NOT EXISTS idx_fullbay_parts_barcode_aliases_gin ON fullbay_import_parts USING GIN (barcode_aliases);
 CREATE INDEX IF NOT EXISTS idx_fullbay_parts_internal_barcode_lower ON fullbay_import_parts (lower(internal_barcode));
 CREATE INDEX IF NOT EXISTS idx_fullbay_parts_part_number_lower ON fullbay_import_parts (lower(part_number));
 ...
 ALTER TABLE customer_units ADD COLUMN IF NOT EXISTS samsara_latitude NUMERIC;
 ALTER TABLE customer_units ADD COLUMN IF NOT EXISTS samsara_longitude NUMERIC;
 ALTER TABLE customer_units ADD COLUMN IF NOT EXISTS samsara_speed_mph NUMERIC;
 ALTER TABLE customer_units ADD COLUMN IF NOT EXISTS samsara_ignition_status TEXT;
 ALTER TABLE customer_units ADD COLUMN IF NOT EXISTS samsara_last_seen_at TIMESTAMPTZ;
 CREATE UNIQUE INDEX IF NOT EXISTS customer_units_samsara_vehicle_uidx ON customer_units(samsara_vehicle_id) WHERE samsara_vehicle_id IS NOT NULL;
...
}catch(e){console.error('[Samsara all units]',e);res.status(500).json({ok:false,error:'Could not load ITTR units',detail:e.message||String(e),build:'24.24.3'})}});
...
app.post("/api/parts/inventory-count/:sessionId/scan",auth,managerPermission("inventory"),async(req,res,next)=>{try{
 ...
 let pr=await db.query(`SELECT *,coalesce(quantity,0) system_qty FROM fullbay_import_parts WHERE lower(coalesce(internal_barcode,''))=lower($1) OR lower(coalesce(part_number,''))=lower($1) OR barcode_aliases @> jsonb_build_array($1::text) LIMIT 1`,[code]);
 if(!pr.rowCount)pr=await db.query(`SELECT *,coalesce(quantity,0) system_qty FROM fullbay_import_parts WHERE EXISTS(SELECT 1 FROM jsonb_array_elements_text(coalesce(barcode_aliases,'[]'::jsonb)) a WHERE lower(a)=lower($1)) LIMIT 1`,[code]);
...
app.get("/api/customer-units/:id/profile",auth,managerPermission("customers"),async(req,res,next)=>{try{
 ...
 let invoiceHistory=[];
 try{
  const ir=await db.query(`SELECT i.id,i.invoice_number,i.status,i.invoice_date,i.created_at,i.finalized_at,i.sent_at,i.unit_number,i.vin,i.mileage,i.total,i.subtotal,i.tax AS tax_amount,i.discount,(coalesce(i.shop_supplies,0)+coalesce(i.environmental_fee,0)) AS additional_fees,0::numeric AS other_charges,i.work_order_id,i.service_order_id,
...
app.get("/api/admin/customer-crm-diagnostics",auth,adminOnly,async(req,res)=>{
 const out={ok:false,version:"24.24.3",tables:{},columns:{},counts:{},sync:null,error:""};
 try{
  const db=requireDb();
...
// ===== v24.24.3: Samsara + duplicate customer management =====
...
async function samsaraAllVehicles(){const token=String(process.env.SAMSARA_API_TOKEN||'').trim();if(!token)throw Object.assign(new Error('Samsara is not configured. Add SAMSARA_API_TOKEN in Railway.'),{status:503});let after='',items=[],guard=0;do{const u=new URL('https://api.samsara.com/fleet/vehicles');u.searchParams.set('limit','512');if(after)u.searchParams.set('after',after);const r=await fetch(u,{headers:{Authorization:`Bearer ${token}`,Accept:'application/json'}}),text=await r.text();let d={};try{d=JSON.parse(text)}catch{}if(!r.ok)throw Object.assign(new Error(r.status===401?'Samsara token was rejected. Check SAMSARA_API_TOKEN in Railway.':(d?.message||d?.error||text||`Samsara HTTP ${r.status}`)),{status:r.status});items.push(...(Array.isArray(d?.data)?d.data:[]));after=String(d?.pagination?.endCursor||'');guard++}while(after&&guard<100);return items}
async function samsaraMatchData(db){const vehicles=await samsaraAllVehicles(),units=(await db.query(`SELECT u.id,u.customer_id,u.unit_number,u.vin,u.samsara_vehicle_id,c.name customer_name FROM customer_units u LEFT JOIN fullbay_import_customers c ON c.id=u.customer_id`)).rows,byId=new Map(units.filter(x=>x.samsara_vehicle_id).map(x=>[String(x.samsara_vehicle_id),x])),byVin=new Map(),byUnit=new Map();for(const u of units){const v=samsaraNormVin(u.vin),n=samsaraNormUnit(u.unit_number);if(v.length===17){if(!byVin.has(v))byVin.set(v,[]);byVin.get(v).push(u)}if(n){if(!byUnit.has(n))byUnit.set(n,[]);byUnit.get(n).push(u)}}return{vehicles,units,match(raw){const v=samsaraSnapshot(raw);let a=[],method='';if(v.id&&byId.has(v.id)){a=[byId.get(v.id)];method='linked'}else if(v.vin.length===17&&(byVin.get(v.vin)||[]).length){a=byVin.get(v.vin);method='vin'}else if(v.unit&&(byUnit.get(samsaraNormUnit(v.unit))||[]).length){a=byUnit.get(samsaraNormUnit(v.unit));method='unit'}return{v,a,method}}}}


app.get('/api/samsara/diagnostics',auth,managerPermission('customers'),async(req,res)=>{const out={ok:false,build:'24.24.3',tokenConfigured:!!String(process.env.SAMSARA_API_TOKEN||'').trim(),database:false,columns:{},samsaraApi:false,vehicleCount:0,errors:[]};try{const db=requireDb();await db.query('SELECT 1');out.database=true;const cq=await db.query(`SELECT column_name FROM information_schema.columns WHERE table_schema='public' AND table_name='customer_units'`);const set=new Set(cq.rows.map(x=>x.column_name));for(const n of ['id','customer_id','unit_number','vin','samsara_vehicle_id','samsara_synced_at','samsara_driver_name','samsara_location','samsara_latitude','samsara_longitude','samsara_speed_mph','samsara_ignition_status','samsara_last_seen_at'])out.columns[n]=set.has(n);try{const v=await samsaraAllVehicles();out.samsaraApi=true;out.vehicleCount=v.length}catch(e){out.errors.push('Samsara API: '+(e.message||String(e)))}out.ok=out.database&&out.samsaraApi&&Object.values(out.columns).every(Boolean)}catch(e){out.errors.push('Database: '+(e.message||String(e)))}res.status(out.ok?200:200).json(out)});

app.get('/api/samsara/integration-status',auth,managerPermission('customers'),async(req,res)=>{res.json({ok:true,build:'24.24.3',configured:!!String(process.env.SAMSARA_API_TOKEN||'').trim(),unitSync:true})});
app.get('/api/samsara/unit-sync/preview',auth,managerPermission('customers'),async(req,res,next)=>{try{const d=await samsaraMatchData(requireDb()),items=d.vehicles.map(x=>{const m=d.match(x);return{...m.v,matchMethod:m.method,matchStatus:m.a.length===1?'matched':m.a.length>1?'ambiguous':'unmatched',matches:m.a.map(u=>({id:u.id,customer:u.customer_name,unit:u.unit_number,vin:u.vin}))}});res.json({ok:true,totalSamsara:items.length,totalIttr:d.units.length,matched:items.filter(x=>x.matchStatus==='matched').length,ambiguous:items.filter(x=>x.matchStatus==='ambiguous').length,unmatched:items.filter(x=>x.matchStatus==='unmatched').length,items})}catch(e){console.error('[Samsara preview]',e);res.status(Number(e.status)||500).json({ok:false,error:'Samsara preview failed',detail:e.message||String(e),build:'24.24.3'})}});
app.post('/api/samsara/unit-sync',auth,managerPermission('customers'),async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const d=await samsaraMatchData(db);let linked=0,updated=0,ambiguous=0,unmatched=0;for(const raw of d.vehicles){const m=d.match(raw),v=m.v;if(m.a.length!==1){m.a.length>1?ambiguous++:unmatched++;continue}const u=m.a[0];await db.query(`UPDATE customer_units SET samsara_vehicle_id=$2,samsara_synced_at=now(),samsara_driver_name=nullif($3,''),samsara_location=nullif($4,''),samsara_latitude=$5,samsara_longitude=$6,samsara_speed_mph=$7,samsara_ignition_status=nullif($8,''),samsara_last_seen_at=CASE WHEN $9::text IS NULL OR $9::text='' THEN samsara_last_seen_at ELSE $9::timestamptz END WHERE id=$1::bigint`,[u.id,v.id,v.driverName,v.location,v.latitude,v.longitude,v.speedMph,v.ignition,v.lastSeen]);updated++;if(m.method!=='linked')linked++}await db.query('COMMIT');await audit(req.user.username,'samsara_units_synced',{vehicles:d.vehicles.length,linked,updated,ambiguous,unmatched});res.json({ok:true,total:d.vehicles.length,linked,updated,ambiguous,unmatched})}catch(e){try{await db.query('ROLLBACK')}catch{}console.error('[Samsara sync]',e);res.status(Number(e.status)||500).json({ok:false,error:'Samsara sync failed',detail:e.message||String(e),build:'24.24.3'})}finally{db.release()}});
app.get('/api/samsara/all-units',auth,managerPermission('customers'),async(req,res,next)=>{try{const q=await requireDb().query(`SELECT u.id,u.customer_id,u.unit_number,u.vin,u.year,u.make,u.model,u.plate,u.mileage,u.samsara_vehicle_id,u.samsara_synced_at,u.samsara_driver_name,u.samsara_location,u.samsara_speed_mph,u.samsara_ignition_status,u.samsara_last_seen_at,c.name customer_name FROM customer_units u LEFT JOIN fullbay_import_customers c ON c.id=u.customer_id ORDER BY c.name,u.unit_number`);res.json({ok:true,items:q.rows.map(x=>({...x,samsaraStatus:x.samsara_vehicle_id?'connected':'not_connected'}))})}catch(e){next(e)}});
...
// v24.24.3 — safe cleanup of incomplete imported Fullbay service history only.
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.24.3",backend:"24.24.3",build:"ITTR-24.24.3-PRINT-LEGAL-PART-LOOKUP-20260914"}));
app.get("/api/health",async(req,res)=>{let db=false;try{if(pool){await pool.query("SELECT 1");db=true}}catch{}res.json({ok:true,db,aiConfigured:Boolean(openRouterClient||client),aiProvider:openRouterClient?"openrouter":client?"openai":"none",version:"24.24.3",photoStorageConfigured:r2Configured})});
