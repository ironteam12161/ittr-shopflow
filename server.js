...
async function initDb(){
 ...
 CREATE TABLE IF NOT EXISTS customer_invoice_lines(
   ...
   unit_cost NUMERIC DEFAULT 0, taxable BOOLEAN DEFAULT FALSE, discount NUMERIC DEFAULT 0, line_total NUMERIC DEFAULT 0,
...
app.get("/api/customer-units/:id/profile",auth,adminOnly,async(req,res,next)=>{try{
 ...
 let invoiceHistory=[];
 try{const ir=await db.query(`SELECT id,invoice_number,work_order_id,service_order_id,unit_number,vin,mileage,status,invoice_date,total,balance_due FROM customer_invoices WHERE status<>'void' AND ((unit_id=$1::bigint) OR (lower(coalesce(unit_number,''))=lower($2::text) AND (customer_id=$3::bigint OR lower(customer_name)=lower($4::text)))) ORDER BY invoice_date DESC,id DESC LIMIT 500`,[unit.id,unit.unit_number||'',unit.customer_id||null,unit.customer_name||'']);invoiceHistory=ir.rows.map(x=>({source:'invoice',id:x.service_order_id||x.work_order_id||x.invoice_number,invoice:x.invoice_number,invoiceId:x.id,unit:x.unit_number,vin:x.vin,completedAt:x.invoice_date,status:`Invoice ${x.status}`,mileage:x.mileage,totalAmount:Number(x.total||0),tasks:[{t:`Invoice ${x.invoice_number}`}]}))}catch(e){console.error('Vehicle invoice history warning:',e?.message)}
 const history=[...fullbayHistory,...ittrHistory,...invoiceHistory].sort((a,b)=>new Date(b.completedAt||b.date||0)-new Date(a.completedAt||a.date||0));
...
app.get("/api/admin/customer-crm-diagnostics",auth,adminOnly,async(req,res)=>{
 const out={ok:false,version:"24.6.9",tables:{},columns:{},counts:{},sync:null,error:""};
 try{
  const db=requireDb();
...
app.get("/api/build",(req,res)=>res.json({frontendExpected:"24.6.9",backend:"24.6.9",build:"ITTR-24.6.9-PRO-INVOICE-SYNC-TAX-20260914"}));
app.get("/api/health",async(req,res)=>{let db=false;try{if(pool){await pool.query("SELECT 1");db=true}}catch{}res.json({ok:true,db,aiConfigured:Boolean(openRouterClient||client),aiProvider:openRouterClient?"openrouter":client?"openai":"none",version:"24.6.9",photoStorageConfigured:r2Configured})});
...
async function recalcInvoice(db,id){const lr=await db.query(`SELECT coalesce(sum(greatest(0,quantity*unit_price-discount)),0) subtotal,coalesce(sum(CASE WHEN taxable THEN greatest(0,quantity*unit_price-discount) ELSE 0 END),0) taxable FROM customer_invoice_lines WHERE invoice_id=$1::bigint`,[id]);const inv=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv)return null;const linesSubtotal=invoiceMoney(lr.rows[0].subtotal),taxable=invoiceMoney(lr.rows[0].taxable),fees=invoiceMoney(inv.shop_supplies)+invoiceMoney(inv.environmental_fee),discount=Math.max(0,invoiceMoney(inv.discount)),subtotal=Math.max(0,linesSubtotal+fees-discount),tax=inv.tax_exempt?0:invoiceMoney(Math.max(0,taxable-Math.min(discount,taxable))*invoiceMoney(inv.tax_rate)/100),total=invoiceMoney(subtotal+tax),paid=invoiceMoney(inv.amount_paid),balance=invoiceMoney(Math.max(0,total-paid));let status=inv.status;if(status!=='void'&&status!=='draft'){status=balance<=0&&total>0?'paid':paid>0?'partial':status==='paid'?'sent':status}await db.query(`UPDATE customer_invoices SET subtotal=$2::numeric,taxable_subtotal=$3::numeric,tax=$4::numeric,total=$5::numeric,balance_due=$6::numeric,status=$7::text,paid_at=CASE WHEN $7::text='paid' THEN coalesce(paid_at,now()) ELSE NULL END,updated_at=now() WHERE id=$1::bigint`,[id,subtotal,taxable,tax,total,balance,status]);return (await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint',[id])).rows[0]}
async function getInvoiceBundle(db,id){const inv=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint',[id])).rows[0];if(!inv)return null;const lines=(await db.query('SELECT * FROM customer_invoice_lines WHERE invoice_id=$1::bigint ORDER BY sort_order,id',[id])).rows;const payments=(await db.query('SELECT * FROM customer_invoice_payments WHERE invoice_id=$1::bigint ORDER BY paid_at,id',[id])).rows;return {invoice:inv,lines,payments}}
const IL_RATE_FILE='https://tax.illinois.gov/content/dam/soi/en/web/tax/research/taxrates/documents/salestaxrates/ordmache-current.txt';
let ilRateCache={at:0,text:''};
app.get('/api/tax/illinois-rate',auth,adminOnly,async(req,res,next)=>{try{const city=String(req.query.city||process.env.SHOP_TAX_CITY||'').trim().toUpperCase(),county=String(req.query.county||process.env.SHOP_TAX_COUNTY||'').trim().toUpperCase();if(!city)return res.status(400).json({error:'Illinois city is required for tax lookup.'});if(!ilRateCache.text||Date.now()-ilRateCache.at>6*3600e3){const rr=await fetch(IL_RATE_FILE,{headers:{'User-Agent':'ITTR-ShopFlow/24.6.9'}});if(!rr.ok)throw new Error(`IDOR rate file HTTP ${rr.status}`);ilRateCache={at:Date.now(),text:await rr.text()}}const lines=ilRateCache.text.split(/\r?\n/).filter(Boolean),matches=lines.filter(line=>line.slice(10,35).trim().toUpperCase()===city&&(!county||line.slice(35,60).trim().toUpperCase()===county));if(!matches.length)return res.status(404).json({error:`No Illinois IDOR rate record found for ${city}${county?', '+county:''}. Use the official MyTax Illinois address Tax Rate Finder.`,officialFinder:'https://mytax.illinois.gov/'});const parsed=matches.map(line=>{const override=line.slice(60,61)==='Y',effective=line.slice(61,69),raw=line.slice(69,74),rate=Number(raw)/1000;return {locationId:line.slice(0,10).trim(),city:line.slice(10,35).trim(),county:line.slice(35,60).trim(),addressOverrideRequired:override,effectiveDate:effective,rate:Number.isFinite(rate)?rate:null}});const safe=parsed.filter(x=>!x.addressOverrideRequired&&x.rate!=null);if(safe.length===1)return res.json({ok:true,rate:safe[0].rate,record:safe[0],source:'Illinois Department of Revenue machine-readable rate file',sourceUrl:IL_RATE_FILE});return res.json({ok:false,addressRequired:true,matches:parsed,source:'Illinois Department of Revenue',officialFinder:'https://mytax.illinois.gov/',message:'This jurisdiction requires an address-specific Illinois Tax Rate Finder lookup. ITTR will not guess the rate.'})}catch(e){next(e)}});
app.get('/api/invoices',auth,adminOnly,async(req,res,next)=>{try{const status=String(req.query.status||'').trim();const q=await requireDb().query(`SELECT * FROM customer_invoices WHERE ($1::text='' OR status=$1::text) ORDER BY invoice_date DESC,id DESC LIMIT 500`,[status]);res.json({items:q.rows})}catch(e){next(e)}});
app.get('/api/invoices/:id',auth,adminOnly,async(req,res,next)=>{try{const x=await getInvoiceBundle(requireDb(),req.params.id);if(!x)return res.status(404).json({error:'Invoice not found.'});res.json(x)}catch(e){next(e)}});
app.post('/api/invoices/from-work-order/:woId',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const woId=String(req.params.woId);const ex=await db.query("SELECT id FROM customer_invoices WHERE work_order_id=$1::text AND status<>'void'",[woId]);if(ex.rowCount){await db.query('ROLLBACK');return res.json({ok:true,id:ex.rows[0].id,existing:true})}const sq=await db.query("SELECT payload FROM app_state WHERE state_key='shopflow'");const sf=sq.rows[0]?.payload||{};const w=(Array.isArray(sf.workorders)?sf.workorders:[]).find(x=>String(x?.id)===woId);if(!w){await db.query('ROLLBACK');return res.status(404).json({error:'Work order not found.'})}if(String(w.status)!=='Completed'){await db.query('ROLLBACK');return res.status(409).json({error:'Complete the work order before creating its invoice.'})}let cust=null;if(w.customerId){cust=(await db.query('SELECT * FROM fullbay_import_customers WHERE id=$1::bigint',[w.customerId])).rows[0]||null}if(!cust&&w.customer){cust=(await db.query('SELECT * FROM fullbay_import_customers WHERE lower(customer_name)=lower($1::text) LIMIT 1',[w.customer])).rows[0]||null}const num=await nextInvoiceNumber(db),rate=invoiceMoney(cust?.default_labor_rate||req.body?.laborRate||0),terms=String(cust?.credit_terms||'Due on Receipt'),days=/30/.test(terms)?30:/15/.test(terms)?15:/45/.test(terms)?45:0;const ins=await db.query(`INSERT INTO customer_invoices(invoice_number,work_order_id,customer_id,customer_name,unit_number,vin,po_number,invoice_date,due_date,terms,tax_rate,default_labor_rate,tax_exempt,tax_exempt_number,created_by) VALUES($1,$2,$3::bigint,$4,$5,$6,$7,CURRENT_DATE,CURRENT_DATE+$8::int,$9,$10::numeric,$11::numeric,$12::boolean,$13,$14) RETURNING id`,[num,woId,cust?.id||null,w.customer||cust?.customer_name||'Customer',w.unit||'',w.vin||'',w.poNumber||'',days,terms,Number(req.body?.taxRate||0),rate,cust?.taxable===false, String(cust?.tax_exempt_number||''),req.user.username]);const id=ins.rows[0].id;const sessions=(await db.query(`SELECT task_uid,sum(extract(epoch from (coalesce(ended_at,now())-started_at))/3600.0) hours FROM task_time_sessions WHERE work_order_id=$1::text AND ended_at IS NOT NULL GROUP BY task_uid`,[woId])).rows;const hm=new Map(sessions.map(x=>[String(x.task_uid),Number(x.hours||0)]));let order=0;for(const t of (Array.isArray(w.tasks)?w.tasks:[])){const job=String(t.t||'Repair'),hours=Math.round((hm.get(String(t.uid||''))||0)*100)/100;const laborIns=await db.query(`INSERT INTO customer_invoice_lines(invoice_id,sort_order,job_uid,job_name,line_type,description,quantity,unit_price,unit_cost,taxable,line_total,metadata,parent_line_id) VALUES($1,$2,$3,$4,'labor',$5,$6::numeric,$7::numeric,0,false,($6::numeric*$7::numeric),$8::jsonb,NULL) RETURNING id`,[id,++order,String(t.uid||''),job,job,hours,rate,JSON.stringify({outcome:t.outcome||'',note:t.outcomeNote||''})]);const laborId=laborIns.rows[0].id;for(const part of (Array.isArray(t.parts)?t.parts:[])){await db.query(`INSERT INTO customer_invoice_lines(invoice_id,sort_order,job_uid,job_name,line_type,description,part_number,quantity,unit_price,unit_cost,taxable,line_total,metadata,parent_line_id) VALUES($1,$2,$3,$4,'part',$5,$6,$7::numeric,$8::numeric,$9::numeric,$10::boolean,($7::numeric*$8::numeric),$11::jsonb,$12::bigint)`,[id,++order,String(t.uid||''),job,String(part.description||part.partNumber||'Part'),String(part.partNumber||''),Number(part.qty||1),Number(part.unitPrice||0),Number(part.unitCost||0),part.sellTaxable!==false,JSON.stringify({inventoryPartId:part.inventoryPartId||null,sourcePartId:part.id||null}),laborId])}}await recalcInvoice(db,id);await db.query('COMMIT');await audit(req.user.username,'invoice_created',{invoiceId:id,workOrderId:woId});res.json({ok:true,id})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.post('/api/invoices',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const b=req.body||{},num=await nextInvoiceNumber(db);const q=await db.query(`INSERT INTO customer_invoices(invoice_number,customer_id,customer_name,unit_id,unit_number,vin,mileage,po_number,invoice_date,due_date,terms,tax_rate,default_labor_rate,tax_exempt,tax_exempt_number,tax_exempt_reason,customer_note,internal_note,created_by) VALUES($1,$2::bigint,$3,$4::bigint,$5,$6,$7::numeric,$8,coalesce($9::date,CURRENT_DATE),coalesce($10::date,CURRENT_DATE),$11,$12::numeric,$13::numeric,$14::boolean,$15,$16,$17,$18,$19) RETURNING id`,[num,b.customerId||null,String(b.customerName||'Customer'),b.unitId||null,String(b.unitNumber||''),String(b.vin||''),Number(b.mileage||0)||null,String(b.poNumber||''),b.invoiceDate||null,b.dueDate||null,String(b.terms||'Due on Receipt'),Number(b.taxRate||0),Number(b.defaultLaborRate||0),b.taxExempt===true,String(b.taxExemptNumber||''),String(b.taxExemptReason||''),String(b.customerNote||''),String(b.internalNote||''),req.user.username]);await recalcInvoice(db,q.rows[0].id);await db.query('COMMIT');res.json({ok:true,id:q.rows[0].id})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.put('/api/invoices/:id',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,cur=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!cur){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(!['draft','sent','partial'].includes(cur.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Paid or void invoices are locked. Reopen a paid invoice with a reason before correcting it.'})}const b=req.body||{};await db.query(`UPDATE customer_invoices SET customer_name=$2,unit_number=$3,vin=$4,po_number=$5,mileage=$15::numeric,invoice_date=$6::date,due_date=$7::date,terms=$8,tax_rate=$9::numeric,shop_supplies=$10::numeric,environmental_fee=$11::numeric,discount=$12::numeric,customer_note=$13,internal_note=$14,default_labor_rate=$16::numeric,tax_exempt=$17::boolean,tax_exempt_number=$18,tax_exempt_reason=$19,tax_rate_source=$20,tax_location_city=$21,tax_location_county=$22,updated_at=now() WHERE id=$1::bigint`,[id,String(b.customerName??cur.customer_name),String(b.unitNumber??cur.unit_number??''),String(b.vin??cur.vin??''),String(b.poNumber??cur.po_number??''),b.invoiceDate||cur.invoice_date,b.dueDate||cur.due_date,String(b.terms??cur.terms??''),Number(b.taxRate??cur.tax_rate??0),Number(b.shopSupplies??cur.shop_supplies??0),Number(b.environmentalFee??cur.environmental_fee??0),Number(b.discount??cur.discount??0),String(b.customerNote??cur.customer_note??''),String(b.internalNote??cur.internal_note??''),Number(b.mileage??cur.mileage??0)||null,Number(b.defaultLaborRate??cur.default_labor_rate??0),b.taxExempt===true,String(b.taxExemptNumber??cur.tax_exempt_number??''),String(b.taxExemptReason??cur.tax_exempt_reason??''),String(b.taxRateSource??cur.tax_rate_source??''),String(b.taxLocationCity??cur.tax_location_city??''),String(b.taxLocationCounty??cur.tax_location_county??'')]);await recalcInvoice(db,id);await db.query('COMMIT');await audit(req.user.username,'invoice_updated',{invoiceId:id,status:cur.status});res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.post('/api/invoices/:id/lines',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,b=req.body||{},inv=(await db.query('SELECT status,default_labor_rate FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(!['draft','sent','partial'].includes(inv.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is locked.'})}const typ=['labor','part','fee','sublet','other'].includes(b.lineType)?b.lineType:'other';let parentLineId=b.parentLineId?Number(b.parentLineId):null,jobUid=String(b.jobUid||''),jobName=String(b.jobName||'');if(parentLineId){const p=(await db.query(`SELECT id,job_uid,job_name FROM customer_invoice_lines WHERE id=$1::bigint AND invoice_id=$2::bigint AND line_type='labor'`,[parentLineId,id])).rows[0];if(!p){await db.query('ROLLBACK');return res.status(409).json({error:'Parent labor operation not found.'})}jobUid=String(p.job_uid||'');jobName=String(p.job_name||'');if(typ==='labor')parentLineId=null}const q=await db.query(`INSERT INTO customer_invoice_lines(invoice_id,sort_order,job_uid,job_name,line_type,description,part_number,quantity,unit_price,unit_cost,taxable,discount,line_total,parent_line_id) VALUES($1::bigint,(SELECT coalesce(max(sort_order),0)+1 FROM customer_invoice_lines WHERE invoice_id=$1::bigint),$2,$3,$4,$5,$6,$7::numeric,$8::numeric,$9::numeric,$10::boolean,$11::numeric,greatest(0,$7::numeric*$8::numeric-$11::numeric),$12::bigint) RETURNING id`,[id,jobUid,jobName,typ,String(b.description??''),String(b.partNumber||''),Number(b.quantity||1),Number(b.unitPrice??(typ==='labor'?inv.default_labor_rate:0)),Number(b.unitCost||0),b.taxable===true,Number(b.discount||0),parentLineId]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true,id:q.rows[0].id})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.put('/api/invoices/:id/lines/:lineId',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const b=req.body||{},id=req.params.id;const inv=(await db.query('SELECT status FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv||!['draft','sent','partial'].includes(inv.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is locked or missing.'})}const typ=['labor','part','fee','sublet','other'].includes(b.lineType)?b.lineType:'other';let parentLineId=typ==='labor'?null:(b.parentLineId?Number(b.parentLineId):null),jobName=String(b.jobName||''),jobUid=String(b.jobUid||'');if(parentLineId){const p=(await db.query(`SELECT job_uid,job_name FROM customer_invoice_lines WHERE id=$1::bigint AND invoice_id=$2::bigint AND line_type='labor'`,[parentLineId,id])).rows[0];if(!p){await db.query('ROLLBACK');return res.status(409).json({error:'Parent labor operation not found.'})}jobName=String(p.job_name||jobName);jobUid=String(p.job_uid||jobUid)}await db.query(`UPDATE customer_invoice_lines SET job_uid=$3,job_name=$4,line_type=$5,description=$6,part_number=$7,quantity=$8::numeric,unit_price=$9::numeric,unit_cost=$10::numeric,taxable=$11::boolean,discount=$12::numeric,line_total=greatest(0,$8::numeric*$9::numeric-$12::numeric),parent_line_id=$13::bigint WHERE id=$2::bigint AND invoice_id=$1::bigint`,[id,req.params.lineId,jobUid,jobName,typ,String(b.description||''),String(b.partNumber||''),Number(b.quantity||0),Number(b.unitPrice||0),Number(b.unitCost||0),b.taxable===true,Number(b.discount||0),parentLineId]);if(typ==='labor')await db.query(`UPDATE customer_invoice_lines SET job_uid=$3::text,job_name=$4::text WHERE invoice_id=$1::bigint AND parent_line_id=$2::bigint`,[id,req.params.lineId,jobUid,jobName]);await recalcInvoice(db,id);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.delete('/api/invoices/:id/lines/:lineId',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const inv=(await db.query('SELECT status FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[req.params.id])).rows[0];if(!inv||!['draft','sent','partial'].includes(inv.status)){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is locked or missing.'})}const target=(await db.query('SELECT line_type FROM customer_invoice_lines WHERE id=$1::bigint AND invoice_id=$2::bigint',[req.params.lineId,req.params.id])).rows[0];if(target?.line_type==='labor')await db.query('DELETE FROM customer_invoice_lines WHERE parent_line_id=$1::bigint AND invoice_id=$2::bigint',[req.params.lineId,req.params.id]);await db.query('DELETE FROM customer_invoice_lines WHERE id=$1::bigint AND invoice_id=$2::bigint',[req.params.lineId,req.params.id]);await recalcInvoice(db,req.params.id);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.post('/api/invoices/:id/finalize',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const inv=await recalcInvoice(db,req.params.id);if(!inv){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}if(invoiceMoney(inv.total)<=0){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice total must be greater than zero.'})}await db.query("UPDATE customer_invoices SET status=CASE WHEN amount_paid>0 THEN 'partial' ELSE 'sent' END,finalized_at=coalesce(finalized_at,now()),sent_at=coalesce(sent_at,now()),updated_at=now() WHERE id=$1::bigint",[req.params.id]);await db.query('COMMIT');res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.post('/api/invoices/:id/payments',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const id=req.params.id,amount=invoiceMoney(req.body?.amount);if(amount<=0){await db.query('ROLLBACK');return res.status(400).json({error:'Payment amount must be greater than zero.'})}const inv=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[id])).rows[0];if(!inv||inv.status==='void'){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is missing or void.'})}if(amount>invoiceMoney(inv.balance_due)+.01){await db.query('ROLLBACK');return res.status(409).json({error:'Payment cannot exceed the balance due.'})}await db.query(`INSERT INTO customer_invoice_payments(invoice_id,amount,method,reference,note,paid_at,received_by) VALUES($1::bigint,$2::numeric,$3,$4,$5,coalesce($6::timestamptz,now()),$7)`,[id,amount,String(req.body?.method||'Other'),String(req.body?.reference||''),String(req.body?.note||''),req.body?.paidAt||null,req.user.username]);await db.query('UPDATE customer_invoices SET amount_paid=amount_paid+$2::numeric,updated_at=now() WHERE id=$1::bigint',[id,amount]);await recalcInvoice(db,id);await db.query('COMMIT');await audit(req.user.username,'invoice_payment',{invoiceId:id,amount,method:req.body?.method});res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.post('/api/invoices/:id/void',auth,adminOnly,async(req,res,next)=>{try{const q=await requireDb().query("UPDATE customer_invoices SET status='void',updated_at=now() WHERE id=$1::bigint AND amount_paid=0 RETURNING id",[req.params.id]);if(!q.rowCount)return res.status(409).json({error:'Paid invoices cannot be voided until payments are reconciled.'});res.json({ok:true})}catch(e){next(e)}});
app.post('/api/invoices/:id/reopen',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const reason=String(req.body?.reason||'').trim();if(reason.length<3){await db.query('ROLLBACK');return res.status(400).json({error:'A correction reason is required.'})}const inv=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[req.params.id])).rows[0];if(!inv||inv.status==='void'){await db.query('ROLLBACK');return res.status(409).json({error:'Invoice is missing or void.'})}const nextStatus=Number(inv.amount_paid||0)>0?'partial':'sent';await db.query(`UPDATE customer_invoices SET status=$2,reopened_at=now(),reopened_by=$3,reopen_reason=$4,updated_at=now() WHERE id=$1::bigint`,[req.params.id,nextStatus,req.user.username,reason]);await db.query('COMMIT');await audit(req.user.username,'invoice_reopened',{invoiceId:req.params.id,reason,previousStatus:inv.status});res.json({ok:true,status:nextStatus})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.delete('/api/invoices/:id/permanent',auth,adminOnly,async(req,res,next)=>{const db=await requireDb().connect();try{await db.query('BEGIN');const inv=(await db.query('SELECT * FROM customer_invoices WHERE id=$1::bigint FOR UPDATE',[req.params.id])).rows[0];if(!inv){await db.query('ROLLBACK');return res.status(404).json({error:'Invoice not found.'})}const pc=Number((await db.query('SELECT count(*) c FROM customer_invoice_payments WHERE invoice_id=$1::bigint',[req.params.id])).rows[0]?.c||0);if(inv.status!=='draft'||pc>0){await db.query('ROLLBACK');return res.status(409).json({error:'Only unpaid draft invoices can be permanently deleted. Finalized invoices must be voided to preserve accounting history.'})}await db.query('DELETE FROM customer_invoice_lines WHERE invoice_id=$1::bigint',[req.params.id]);await db.query('DELETE FROM customer_invoices WHERE id=$1::bigint',[req.params.id]);await db.query('COMMIT');await audit(req.user.username,'invoice_deleted',{invoiceId:req.params.id,invoiceNumber:inv.invoice_number});res.json({ok:true})}catch(e){try{await db.query('ROLLBACK')}catch{}next(e)}finally{db.release()}});
app.get('/api/invoices/:id/pdf',auth,adminOnly,async(req,res,next)=>{try{
 const db=requireDb(),x=await getInvoiceBundle(db,req.params.id);if(!x)return res.status(404).json({error:'Invoice not found.'});const i=x.invoice;
 res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`attachment; filename="${String(i.invoice_number).replace(/[^A-Za-z0-9_-]/g,'_')}.pdf"`);res.setHeader('Cache-Control','private, no-store');
 const doc=new PDFDocument({size:'LETTER',margins:{top:32,left:38,right:38,bottom:24},bufferPages:true,info:{Title:`Invoice ${i.invoice_number}`,Author:'Iron Team Truck & Trailer Repair'}});doc.pipe(res);
 const money=n=>`$${invoiceMoney(n).toFixed(2)}`,L=38,W=536,logoPath=path.join(__dirname,'public','assets','iron-team-logo.png');
 const lineSubtotal=x.lines.reduce((a,l)=>a+invoiceMoney(l.line_total),0),supplies=invoiceMoney(i.shop_supplies),env=invoiceMoney(i.environmental_fee),discount=invoiceMoney(i.discount);
 try{if(fs.existsSync(logoPath))doc.image(logoPath,L,26,{fit:[72,72]})}catch{}
 doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(15).text('IRON TEAM TRUCK & TRAILER REPAIR',L+82,34,{width:300,lineBreak:false});
 doc.font('Helvetica').fontSize(8.5).fillColor('#64748b').text('Heavy-Duty Truck & Trailer Service',L+82,57,{width:300,lineBreak:false});
 doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(26).text('INVOICE',390,31,{width:184,align:'right'});doc.fontSize(10).fillColor('#2563eb').text(i.invoice_number,390,66,{width:184,align:'right'});
 let y=108;doc.roundedRect(L,y,W,70,7).fillAndStroke('#f8fafc','#dbe3ec');doc.fillColor('#334155').font('Helvetica-Bold').fontSize(8).text('BILL TO',L+12,y+10);doc.fillColor('#0f172a').fontSize(11).text(i.customer_name,L+12,y+25,{width:280});doc.font('Helvetica').fontSize(8.2).fillColor('#475569').text(`Unit ${i.unit_number||'—'}  ·  VIN ${i.vin||'—'}${i.mileage?`  ·  ${Number(i.mileage).toLocaleString()} mi`:''}`,L+12,y+47,{width:340});
 const metaX=386,valX=450;doc.font('Helvetica-Bold').fontSize(7.5).fillColor('#334155').text('DATE',metaX,y+10);doc.font('Helvetica').text(String(i.invoice_date||'').slice(0,10),valX,y+10);doc.font('Helvetica-Bold').text('DUE',metaX,y+27);doc.font('Helvetica').text(String(i.due_date||'').slice(0,10),valX,y+27);doc.font('Helvetica-Bold').text('PO #',metaX,y+44);doc.font('Helvetica').text(i.po_number||'—',valX,y+44);
 y+=90;const tableHeader=()=>{doc.rect(L,y,W,23).fill('#0f172a');doc.fillColor('#fff').font('Helvetica-Bold').fontSize(7.3).text('SERVICE / DESCRIPTION',L+9,y+8,{width:295});doc.text('QTY/HRS',356,y+8,{width:54,align:'right'});doc.text('RATE/PRICE',414,y+8,{width:70,align:'right'});doc.text('AMOUNT',490,y+8,{width:76,align:'right'});y+=31};tableHeader();
 const labors=x.lines.filter(l=>l.line_type==='labor'),children=x.lines.filter(l=>l.line_type!=='labor');
 for(const labor of labors){if(y>650){doc.addPage();y=42;tableHeader()}doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(9).text(`LABOR  ${labor.description||'Service'}`,L+9,y,{width:292});doc.font('Helvetica').text(Number(labor.quantity||0).toFixed(2),356,y,{width:54,align:'right'});doc.text(money(labor.unit_price),414,y,{width:70,align:'right'});doc.font('Helvetica-Bold').text(money(labor.line_total),490,y,{width:76,align:'right'});y+=19;
  for(const c of children.filter(z=>String(z.parent_line_id||'')===String(labor.id))){if(y>670){doc.addPage();y=42;tableHeader()}const label=c.line_type==='part'?`PART${c.part_number?` · ${c.part_number}`:''}`:String(c.line_type||'CHARGE').toUpperCase();doc.fillColor('#64748b').font('Helvetica-Bold').fontSize(7.7).text(label,L+22,y,{width:118});doc.font('Helvetica').fillColor('#111827').text(c.description||'',L+145,y,{width:165});doc.text(String(Number(c.quantity||0)),356,y,{width:54,align:'right'});doc.text(money(c.unit_price),414,y,{width:70,align:'right'});doc.text(money(c.line_total),490,y,{width:76,align:'right'});y+=18}
  doc.moveTo(L,y+1).lineTo(L+W,y+1).strokeColor('#e2e8f0').stroke();y+=10}
 y=Math.max(y+10,455);if(y>620){doc.addPage();y=70}
 const right=370,labelW=112,valX2=486,valW=80;const summary=(label,val,bold=false)=>{doc.fillColor('#334155').font(bold?'Helvetica-Bold':'Helvetica').fontSize(bold?11:8.8).text(label,right,y,{width:labelW});doc.fillColor('#0f172a').font(bold?'Helvetica-Bold':'Helvetica').text(val,valX2,y,{width:valW,align:'right'});y+=bold?21:16};
 summary('Services & Parts',money(lineSubtotal));if(supplies)summary('Shop Supplies',money(supplies));if(env)summary('Environmental / Other',money(env));if(discount)summary('Discount',`-${money(discount)}`);doc.moveTo(right,y-2).lineTo(L+W,y-2).strokeColor('#cbd5e1').stroke();summary('Subtotal',money(i.subtotal));
 if(i.tax_exempt)summary(`Tax Exempt${i.tax_exempt_number?' · '+i.tax_exempt_number:''}`,'$0.00');else summary(`Sales Tax (${Number(i.tax_rate||0).toFixed(3)}%)`,money(i.tax));summary('TOTAL',money(i.total),true);if(Number(i.amount_paid||0))summary('Paid',`-${money(i.amount_paid)}`);summary('BALANCE DUE',money(i.balance_due),true);
 if(i.customer_note){y+=12;doc.fillColor('#334155').font('Helvetica-Bold').fontSize(8).text('CUSTOMER NOTE',L,y);doc.font('Helvetica').fontSize(8.5).fillColor('#111827').text(i.customer_note,L,y+13,{width:310,height:55,ellipsis:true})}
 const footerNote='PARTS WARRANTY: Any parts warranty is the responsibility of the part manufacturer and is subject to the manufacturer’s warranty terms. Iron Team Truck & Trailer Repair is not responsible for manufacturer defects except as required by law.  •  WHEEL / TIRE SERVICE: After wheel or tire replacement/removal, customer must have wheel fastener torque rechecked after 50 miles of operation.';
 const pages=doc.bufferedPageRange();for(let n=0;n<pages.count;n++){doc.switchToPage(n);doc.moveTo(L,716).lineTo(L+W,716).strokeColor('#cbd5e1').stroke();doc.font('Helvetica-Bold').fontSize(6.7).fillColor('#475569').text('IMPORTANT SERVICE & WARRANTY NOTICE',L,724,{width:W,lineBreak:false});doc.font('Helvetica').fontSize(6.4).fillColor('#64748b').text(footerNote,L,735,{width:W,height:30,ellipsis:true,lineGap:0.5});doc.fontSize(6.5).text(`${i.invoice_number}  ·  Page ${n+1} of ${pages.count}`,L,770,{width:W,align:'center',lineBreak:false})}
 doc.end()
}catch(e){next(e)}});
